...
- 网络路径：
  ...
  - 通过单播 DNS-SD 的 Tailnet（示例域：`jarvis.internal.`），**或**
  - 手动主机/端口（备选）。
...
Gateway 网关在 `local.` 上广播 `_jarvis-gw._tcp`。iOS 应用会自动列出这些。

### Tailnet（跨网络）

如果 mDNS 被阻止，使用单播 DNS-SD 区域（选择一个域；示例：`jarvis.internal.`）和 Tailscale 分割 DNS。
...
```bash
jarvis nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18793/__jarvis__/canvas/"}'
```

注意事项：

- Gateway 网关 canvas 主机服务于 `/__jarvis__/canvas/` 和 `/__jarvis__/a2ui/`。
...
```bash
jarvis nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- 配对提示从未出现：运行 `jarvis nodes pending` 并手动批准。
