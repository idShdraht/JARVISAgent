---
read_when:
  - 你想了解 `jarvis.ai/install.sh` 的工作机制
  - 你想自动化安装（CI / 无头环境）
  - 你想从 GitHub 检出安装
...
JARVIS 提供两个安装器脚本（托管在 `jarvis.ai`）：

- `https://jarvis.ai/install.sh` — "推荐"安装器（默认全局 npm 安装；也可从 GitHub 检出安装）
- `https://jarvis.ai/install-cli.sh` — 无需 root 权限的 CLI 安装器（安装到带有独立 Node 的前缀目录）
- `https://jarvis.ai/install.ps1` — Windows PowerShell 安装器（默认 npm；可选 git 安装）

查看当前参数/行为，运行：

```bash
curl -fsSL https://jarvis.ai/install.sh | bash -s -- --help
```

Windows (PowerShell) 帮助：

```powershell
& ([scriptblock]::Create((iwr -useb https://jarvis.ai/install.ps1))) -?
```

如果安装器完成但在新终端中找不到 `jarvis`，通常是 Node/npm PATH 问题。参见：[安装](/install#nodejs--npm-path-sanity)。
...
- 选择安装方式：
  - `npm`（默认）：`npm install -g jarvis@latest`
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL https://jarvis.ai/install.sh | bash
```
...
在非交互式上下文中（无 TTY / `--no-prompt`），你必须传入 `--install-method git|npm`（或设置 `JARVIS_INSTALL_METHOD`），否则脚本将以退出码 `2` 退出。
...
此脚本将 `jarvis` 安装到前缀目录（默认：`~/.jarvis`），同时在该前缀下安装专用的 Node 运行时，因此可以在不想改动系统 Node/npm 的机器上使用。

帮助：

```bash
curl -fsSL https://jarvis.ai/install-cli.sh | bash -s -- --help
```
...
```powershell
iwr -useb https://jarvis.ai/install.ps1 | iex -InstallMethod git
```

```powershell
iwr -useb https://jarvis.ai/install.ps1 | iex -InstallMethod git -GitDir "C:\\jarvis"
```

环境变量：

- `JARVIS_INSTALL_METHOD=git|npm`
- `JARVIS_GIT_DIR=...`
