...
通过一键模板在 Railway 上部署 JARVIS，并在浏览器中完成设置。
...
- `https://<your-railway-domain>/jarvis` — 控制面板 UI
...
- `JARVIS_STATE_DIR=/data/.jarvis`（推荐）
- `JARVIS_WORKSPACE_DIR=/data/workspace`（推荐）
- `JARVIS_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 JARVIS 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
