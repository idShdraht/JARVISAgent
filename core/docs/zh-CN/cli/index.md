...
注意：插件可以添加额外的顶级命令（例如 `jarvis voicecall`）。

## 安全

- `jarvis security audit` — 审计配置 + 本地状态中常见的安全隐患。
- `jarvis security audit --deep` — 尽力进行实时 Gateway 网关探测。
- `jarvis security audit --fix` — 收紧安全默认值并 chmod 状态/配置。

## 插件

管理扩展及其配置：

- `jarvis plugins list` — 发现插件（使用 `--json` 获取机器可读输出）。
- `jarvis plugins info <id>` — 显示插件详情。
- `jarvis plugins install <path|.tgz|npm-spec>` — 安装插件（或将插件路径添加到 `plugins.load.paths`）。
- `jarvis plugins enable <id>` / `disable <id>` — 切换 `plugins.entries.<id>.enabled`。
- `jarvis plugins doctor` — 报告插件加载错误。
...
- `jarvis memory status` — 显示索引统计。
- `jarvis memory index` — 重新索引记忆文件。
- `jarvis memory search "<query>"` — 对记忆进行语义搜索。
...
非交互式配置辅助工具（get/set/unset）。不带子命令运行 `jarvis config` 会启动向导。
...
- `channels status`：检查 Gateway 网关可达性和渠道健康状况（`--probe` 运行额外检查；使用 `jarvis health` 或 `jarvis status --deep` 进行 Gateway 网关健康探测）。
- 提示：`channels status` 在检测到常见配置错误时会打印带有建议修复的警告（然后指向 `jarvis doctor`）。
...
当 OAuth/API 凭证可用时，JARVIS 可以显示提供商用量/配额。

显示位置：

- `/status`（可用时添加简短的提供商用量行）
- `jarvis status --usage`（打印完整的提供商明细）
- macOS 菜单栏（上下文下的用量部分）
...
- `gateway status` 在检测到旧版或额外的 Gateway 网关服务时也会显示（`--deep` 添加系统级扫描）。配置文件命名的 JARVIS 服务被视为一等公民，不会被标记为"额外"。
...
`jarvis models` 是 `models status` 的别名。
...
`node` 运行**无头节点主机**或将其作为后台服务管理。参见 [`jarvis node`](/cli/node)。
...
浏览器控制 CLI（专用 Chrome/Brave/Edge/Chromium）。参见 [`jarvis browser`](/cli/browser) 和[浏览器工具](/tools/browser)。
