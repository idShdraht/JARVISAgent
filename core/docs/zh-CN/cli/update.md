...
read_when:
  - 你想安全地更新源码检出
  - 你需要了解 `--update` 简写行为
summary: "`jarvis update` 的 CLI 参考（相对安全的源码更新 + Gateway 网关自动重启）"
title: update
x-i18n:
...
安全更新 JARVIS 并在 stable/beta/dev 渠道之间切换。
...
7. 构建 + 构建控制界面。
8. 运行 `jarvis doctor` 作为最终的"安全更新"检查。
...
- `jarvis doctor`（在 git 检出上会提供先运行更新的选项）
