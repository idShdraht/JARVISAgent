...
- 替代旧版 `gateway` 命令。CLI 入口点：`jarvis gateway`。
...
- `health` — 完整健康快照（与 `jarvis health --json` 形状相同）。
- `status` — 简短摘要。
...
- 如果检测到其他类似 Gateway 网关的服务，CLI 会发出警告，除非它们是 JARVIS 配置文件服务。
  我们仍然建议大多数设置**每台机器一个 Gateway 网关**；使用隔离的配置文件/端口进行冗余或救援机器人。参见[多个 Gateway 网关](/gateway/multiple-gateways)。
  - 清理：`jarvis gateway uninstall`（当前服务）和 `jarvis doctor`（旧版迁移）。
- `gateway install` 在已安装时是无操作的；使用 `jarvis gateway install --force` 重新安装（配置文件/env/路径更改）。
...
JARVIS 在 Linux/WSL2 上默认安装 **systemd 用户服务**。我们
建议单用户机器使用用户服务（更简单的 env，按用户配置）。
对于多用户或常驻服务器使用**系统服务**（无需 lingering，
共享监管）。

`jarvis gateway install` 写入用户单元。`jarvis doctor` 审计
单元并可以将其更新以匹配当前推荐的默认值。

创建 `~/.config/systemd/user/jarvis-gateway[-<profile>].service`：

```
[Unit]
Description=JARVIS Gateway (profile: <profile>, v<version>)
...
```
systemctl --user enable --now jarvis-gateway[-<profile>].service
```

**替代方案（系统服务）** - 对于常驻或多用户服务器，你可以
安装 systemd **系统**单元而不是用户单元（无需 lingering）。
创建 `/etc/systemd/system/jarvis-gateway[-<profile>].service`（复制上面的单元，
切换 `WantedBy=multi-user.target`，设置 `User=` + `WorkingDirectory=`），然后：

```
sudo systemctl daemon-reload
sudo systemctl enable --now jarvis-gateway[-<profile>].service
```
...
- `jarvis gateway health|status` — 通过 Gateway 网关 WS 请求 health/status。
- `jarvis message send --target <num> --message "hi" [--media ...]` — 通过 Gateway 网关发送（对 WhatsApp 是幂等的）。
- `jarvis agent --message "hi" --to <num>` — 运行智能体轮次（默认等待最终结果）。
- `jarvis gateway call <method> --params '{"k":"v"}'` — 用于调试的原始方法调用器。
- `jarvis gateway stop|restart` — 停止/重启受监管的 Gateway 网关服务（launchd/systemd）。
- Gateway 网关辅助子命令假设 `--url` 上有运行中的 Gateway 网关；它们不再自动生成一个。

## 迁移指南

- 淘汰 `jarvis gateway` 和旧版 TCP 控制端口的使用。
