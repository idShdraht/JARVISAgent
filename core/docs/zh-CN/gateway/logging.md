...
- 默认滚动日志文件位于 `/tmp/jarvis/` 下（每天一个文件）：`jarvis-YYYY-MM-DD.log`
  - 日期使用 Gateway 网关主机的本地时区。
- 日志文件路径和级别可以通过 `~/.jarvis/jarvis.json` 配置：
  - `logging.file`
  - `logging.level`
...
`jarvis gateway` 支持每个 Gateway 网关的样式切换：
...
# 显示所有 WS 流量（配对）
jarvis gateway --verbose --ws-log compact

# 显示所有 WS 流量（完整元数据）
jarvis gateway --verbose --ws-log full
```
