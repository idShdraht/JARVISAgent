...
macOS 也可以在**节点模式**下运行：菜单栏应用连接到 Gateway 网关的 WS 服务器，并将其本地 canvas/camera 命令作为节点暴露（因此 `jarvis nodes …` 可以针对这台 Mac 工作）。
...
- `node.pair.*`（CLI：`jarvis nodes pending/approve/reject`）是一个单独的 Gateway 网关拥有的
...
- **批准**：通过 `~/.jarvis/exec-approvals.json` 在节点主机上执行。
...
```bash
jarvis node run --host <gateway-host> --port 18789 --display-name "Build Node"
```
...
- 令牌是 Gateway 网关配置中的 `gateway.auth.token`（Gateway 网关主机上的 `~/.jarvis/jarvis.json`）。
- `jarvis node run` 读取 `JARVIS_GATEWAY_TOKEN` 进行认证。

### 启动节点主机（服务）

```bash
jarvis node install --host <gateway-host> --port 18789 --display-name "Build Node"
jarvis node restart
```
...
- 在 `jarvis node run` / `jarvis node install` 上使用 `--display-name`（持久化在节点上的 `~/.jarvis/node.json` 中）。
- `jarvis nodes rename --node <id|name|ip> --name "Build Node"`（Gateway 网关覆盖）。
...
```bash
jarvis approvals allowlist add --node <id|name|ip> "/usr/bin/uname"
jarvis approvals allowlist add --node <id|name|ip> "/usr/bin/sw_vers"
```

批准存储在节点主机的 `~/.jarvis/exec-approvals.json` 中。
...
```bash
jarvis config set tools.exec.host node
jarvis config set tools.exec.security allowlist
jarvis config set tools.exec.node "<id-or-name>"
```
...
```bash
jarvis nodes invoke --node <idOrNameOrIp> --command canvas.eval --params '{"javaScript":"location.href"}'
```
...
```bash
jarvis nodes canvas snapshot --node <idOrNameOrIp> --format png
jarvis nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
```

### Canvas 控制

```bash
jarvis nodes canvas present --node <idOrNameOrIp> --target https://example.com
jarvis nodes canvas hide --node <idOrNameOrIp>
jarvis nodes canvas navigate https://example.com --node <idOrNameOrIp>
jarvis nodes canvas eval --node <idOrNameOrIp> --js "document.title"
```
...
```bash
jarvis nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello"
jarvis nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl
jarvis nodes canvas a2ui reset --node <idOrNameOrIp>
```
...
```bash
jarvis nodes camera list --node <idOrNameOrIp>
jarvis nodes camera snap --node <idOrNameOrIp>            # 默认：两个朝向（2 个 MEDIA 行）
jarvis nodes camera snap --node <idOrNameOrIp> --facing front
```

视频片段（`mp4`）：

```bash
jarvis nodes camera clip --node <idOrNameOrIp> --duration 10s
jarvis nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
```
...
```bash
jarvis nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10
jarvis nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
```
...
```bash
jarvis nodes location get --node <idOrNameOrIp>
jarvis nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
```
...
```bash
jarvis nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from JARVIS"}'
```
...
```bash
jarvis nodes run --node <idOrNameOrIp> -- echo "Hello from mac node"
jarvis nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
```
...
- 在无头节点主机上，`system.run` 受 exec 批准限制（`~/.jarvis/exec-approvals.json`）。
...
```bash
jarvis config set tools.exec.node "node-id-or-name"
```
...
```bash
jarvis config unset tools.exec.node
jarvis config unset agents.list[0].tools.exec.node
```
...
JARVIS 可以运行**无头节点主机**（无 UI），它连接到 Gateway 网关
...
```bash
jarvis node run --host <gateway-host> --port 18789
```

注意事项：

- 仍然需要配对（Gateway 网关会显示节点批准提示）。
- 节点主机将其节点 id、令牌、显示名称和 Gateway 网关连接信息存储在 `~/.jarvis/node.json` 中。
- Exec 批准通过 `~/.jarvis/exec-approvals.json` 在本地执行
  （参见 [Exec 批准](/tools/exec-approvals)）。
- 在 macOS 上，当配套应用 exec 主机可达时，无头节点主机优先使用它，
  如果应用不可用则回退到本地执行。设置 `JARVIS_NODE_EXEC_HOST=app` 要求
  使用应用，或设置 `JARVIS_NODE_EXEC_FALLBACK=0` 禁用回退。
- 当 Gateway 网关 WS 使用 TLS 时，添加 `--tls` / `--tls-fingerprint`。

## Mac 节点模式

- macOS 菜单栏应用作为节点连接到 Gateway 网关 WS 服务器（因此 `jarvis nodes …` 可以针对这台 Mac 工作）。
