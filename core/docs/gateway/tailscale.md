...
JARVIS can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. JARVIS requires a shared password.
...
- `token` (default when `JARVIS_GATEWAY_TOKEN` is set)
- `password` (shared secret via `JARVIS_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
Control UI/WebSocket auth can use Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password. JARVIS verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
JARVIS only treats a request as Serve when it arrives from loopback with
...
Prefer `JARVIS_GATEWAY_PASSWORD` over committing a password to disk.
...
- Set `gateway.tailscale.resetOnExit` if you want JARVIS to undo `tailscale serve`
