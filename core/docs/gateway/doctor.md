...
`jarvis doctor` is the repair + migration tool for JARVIS. It fixes stale
...
offers to remove them and install the JARVIS service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named JARVIS gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `jarvis doctor --generate-gateway-token` to force token
...
- `jarvis doctor` prompts before rewriting supervisor config.
- `jarvis doctor --yes` accepts the default repair prompts.
- `jarvis doctor --repair` applies recommended fixes without prompts.
- `jarvis doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `jarvis gateway install --force`.
