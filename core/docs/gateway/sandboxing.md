---
summary: "How JARVIS sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
JARVIS can run **tools inside Docker containers** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - noVNC observer access is password-protected by default; JARVIS emits a short-lived token URL that resolves to the observer session.
...
JARVIS mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- JARVIS blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
...
- Use `jarvis sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
