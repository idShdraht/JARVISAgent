...
read_when:
  - Running JARVIS behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of JARVIS
...
- You run JARVIS behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. JARVIS checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. JARVIS extracts the user identity from the configured header
...
- JARVIS can stay on loopback HTTP behind the proxy.
...
If JARVIS itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://jarvis.example.com
    to: http://jarvis-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`jarvis security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update JARVIS config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `jarvis security audit` and review findings
