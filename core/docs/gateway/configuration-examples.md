---
summary: "Schema-accurate configuration examples for common JARVIS setups"
read_when:
  - Learning how to configure JARVIS
...
```json5
{
  agent: { workspace: "~/.jarvis/workspace" },
  channels: { whatsapp: { allowFrom: ["+15555550123"] } },
}
```

Save to `~/.jarvis/jarvis.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.jarvis/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
