...
JARVIS supports additive secret references so credentials do not need to be stored as plaintext in config files.
...
When onboarding runs in interactive mode and you choose secret reference storage, JARVIS performs a fast preflight check before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). JARVIS validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/JARVIS QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
...
When reload-time activation fails after a healthy state, JARVIS enters degraded secrets state.
...
- plaintext values at rest (`jarvis.json`, `auth-profiles.json`, `.env`)
- unresolved refs
...
- lets you select secret-bearing fields in `jarvis.json`
...
JARVIS intentionally does **not** write rollback backups that contain pre-migration plaintext secret values.
...
For static credentials, JARVIS runtime no longer depends on plaintext `auth.json`.
