...
- During heartbeat runs, JARVIS treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `jarvis sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, JARVIS skips the heartbeat run entirely (no model call).
...
```bash
jarvis system event --text "Check for urgent follow-ups" --mode now
```
