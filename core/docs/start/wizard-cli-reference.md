...
read_when:
  - You need detailed behavior for jarvis onboard
...
This page is the full reference for `jarvis onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.jarvis/jarvis.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `jarvis doctor` before continuing.
  ...
  <Step title="Workspace">
    - Default `~/.jarvis/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `jarvis pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `jarvis health`.
    - `jarvis status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.jarvis/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.jarvis/agents/<agentId>/agent/auth-profiles.json`
...
`~/.jarvis/credentials/oauth.json` (or `$JARVIS_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>

## Outputs and internals

Typical fields in `~/.jarvis/jarvis.json`:
...
WhatsApp credentials go under `~/.jarvis/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.jarvis/agents/<agentId>/sessions/`.
...
- Stores it under `~/.jarvis/tools/signal-cli/<version>/`
...
- Command reference: [`jarvis onboard`](/cli/onboard)
