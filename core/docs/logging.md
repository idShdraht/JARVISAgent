...
```bash
jarvis channels logs --channel whatsapp
```
...
- JARVIS exports via **OTLP/HTTP (protobuf)** today.
...
- You can also enable the plugin with `jarvis plugins enable diagnostics-otel`.
...
- **Gateway not reachable?** Run `jarvis doctor` first.
