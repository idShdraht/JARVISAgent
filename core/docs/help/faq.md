---
summary: "Frequently asked questions about JARVIS setup, configuration, and usage"
read_when:
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [ELI5: `jarvis gateway restart` vs `jarvis gateway`](#eli5-jarvis-gateway-restart-vs-jarvis-gateway)
...
7. **Gateway snapshot**
   ...
   ```bash
   jarvis health --json
   jarvis health --verbose   # shows the target URL + config path on errors
   ```
...
- `jarvis status`: quick snapshot of gateway/agent health + basic config.
- `jarvis models status`: checks provider auth + model availability.
- `jarvis doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `jarvis status --all`, `jarvis logs --follow`,
...
### What's the recommended way to install and set up JARVIS
...
If you don't have a global install yet, run it via `pnpm jarvis onboard`.
...
- **Tailscale Serve** (recommended): keep bind loopback, run `jarvis gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
- **Tailnet bind**: run `jarvis gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
...
### Windows install says git not found or jarvis not recognized
...
**2) jarvis is not recognized after install**
...
### How do I install JARVIS on a VPS
...
### Can I ask JARVIS to update itself
...
`jarvis onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run JARVIS with **API keys** (Anthropic/OpenAI/others) or with
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with `jarvis models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `jarvis models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `jarvis models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro or Max)

Yes - via **setup-token**. JARVIS no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so JARVIS can keep replying while a provider is rate-limited.
...
JARVIS supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.3-codex` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. JARVIS fully supports **OpenAI Code (Codex) subscription OAuth**. The onboarding wizard
...
1. Enable the plugin: `jarvis plugins enable google-gemini-cli-auth`
2. Login: `jarvis models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. JARVIS needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.1 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. JARVIS runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
### If I buy a Mac mini to run JARVIS can I connect it to my MacBook Pro
...
- Use `jarvis nodes status` / `jarvis nodes list` to see it.
...
The onboarding wizard accepts `@username` input and resolves it to a numeric ID, but JARVIS authorization uses numeric IDs only.

Safer (no third-party bot):

- DM your bot, then run `jarvis logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different JARVIS instances
...
If you run JARVIS via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
**JARVIS-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run JARVIS on a dedicated machine
...
JARVIS is lightweight. For a basic Gateway + one chat channel:
...
### Can I run JARVIS in a VM and what are the requirements
...
### What is JARVIS in one paragraph

JARVIS is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### What's the value proposition

JARVIS is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for JARVIS
...
### Can JARVIS help with lead gen outreach ads and blogs for a SaaS
...
JARVIS draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

JARVIS is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use JARVIS when you
...
### Can JARVIS run tasks on a schedule or continuously in the background
...
JARVIS memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

JARVIS also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, JARVIS auto-selects a provider when it
...
### Is all data used with JARVIS saved locally
...
### How do I completely uninstall JARVIS
...
`workspace` to the repo root. The JARVIS repo is just source code; keep the
...
JARVIS enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `jarvis doctor --generate-gateway-token`.
...
key. **Recommended:** run `jarvis configure --section web` to store it in
...
### Can the JARVIS browser run headless
...
- Gateway is running: `jarvis gateway status`
- Gateway health: `jarvis status`
- Channel health: `jarvis channels status`
...
### Can two JARVIS instances talk to each other local VPS
...
`jarvis agent --message ... --deliver`, targeting a chat where the other bot
...
- Use `jarvis config set` for small changes.
- Use `jarvis configure` for interactive edits.
...
### How does JARVIS load environment variables
...
`jarvis models status` reports whether **shell env import** is enabled. "Shell env: off"
does **not** mean your env vars are missing - it just means JARVIS won't load
...
### Is there a way to make a team of JARVIS instances one CEO and many agents
...
### How do I completely reset JARVIS but keep it installed
...
headers like `# Heading`), JARVIS skips the heartbeat run to save API calls.
...
No. JARVIS runs on **your own account**, so if you're in the group, JARVIS can see it.
...
### Why doesn't JARVIS reply in a group
...
- Use `jarvis doctor` to spot stray workspaces and profile mismatches.
...
JARVIS's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, JARVIS currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
### What do JARVIS, Flawd, and Krill use for models

- **JARVIS + Flawd:** Anthropic Opus (`anthropic/claude-opus-4-6`) - see [Anthropic](/providers/anthropic).
...
Yes. JARVIS ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
- Run `jarvis agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so JARVIS can keep responding even when a provider is rate-limited or temporarily failing.
...
- **Sanity-check model/auth status**
  - Use `jarvis models status` to see configured models and whether providers are authenticated.
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), JARVIS will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix: JARVIS now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
JARVIS uses provider-prefixed IDs like:
...
JARVIS may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `jarvis models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
...
Use `jarvis gateway status` and trust these lines:
...
JARVIS enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `jarvis gateway --port <port>`.

### How do I run JARVIS in remote mode client connects to a Gateway elsewhere
...
- `jarvis gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
- The Control UI stores the token in browser localStorage key `jarvis.control.settings.v1`.
...
- Still stuck? Run `jarvis status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
If you run the gateway manually, `jarvis gateway --force` can reclaim the port. See [Gateway](/gateway).

### I closed my terminal on Windows how do I restart JARVIS
...
1. Is the Gateway running? `jarvis gateway status`
2. Is the Gateway healthy? `jarvis status`
3. Does the UI have the right token? `jarvis dashboard`
...
### ELI5 jarvis gateway restart vs jarvis gateway

- `jarvis gateway restart`: restarts the **background service** (launchd/systemd).
- `jarvis gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `jarvis gateway` when
...
### Is it safe to expose JARVIS to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM-capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `jarvis pairing approve --channel <channel> [--account <id>] <code>`
  - Pending requests are capped at **3 per channel**; check `jarvis pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
- Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

Run `jarvis doctor` to surface risky DM policies.
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. JARVIS only replies to chats it receives or to explicit sends you trigger.
...
JARVIS blocks **cross-provider** messaging by default. If a tool call is bound
