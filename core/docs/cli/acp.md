...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a JARVIS Gateway.
...
jarvis acp --session agent:main:main --reset-session
```
...
it to drive a JARVIS Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `jarvis acp` over stdio.
