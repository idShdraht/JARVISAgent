...
Use `jarvis secrets` to migrate credentials from plaintext to SecretRefs and keep the active secrets runtime healthy.
...
- `configure` targets secret-bearing fields in `jarvis.json`.
