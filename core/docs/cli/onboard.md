---
summary: "CLI reference for `jarvis onboard` (interactive onboarding wizard)"
read_when:
...
```bash
jarvis onboard
jarvis onboard --flow quickstart
jarvis onboard --flow manual
jarvis onboard --mode remote --remote-url ws://gateway-host:18789
```
...
- Fastest first chat: `jarvis dashboard` (Control UI, no channel setup).
