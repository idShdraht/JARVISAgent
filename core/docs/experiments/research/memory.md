...
read_when:
  - Designing workspace memory (~/.jarvis/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep JARVIS integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.jarvis/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.jarvis/workspace` as canonical human-readable memory.
...
```
~/.jarvis/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in JARVIS**, but keep a separable core library.

### Why integrate into JARVIS?

- JARVIS already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `jarvis memory recall "…" --k 25 --since 30d`
  - `jarvis memory reflect --since 7d`
...
Pragmatic take for `~/.jarvis/workspace`:
