...
This plan defines how JARVIS should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- JARVIS core owns ACP control plane concerns
...
JARVIS should not reimplement ACP transport internals in core.
JARVIS should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in JARVIS, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under JARVIS state dir
...
- ACP event stream is normalized to JARVIS reply chunks
...
For the first production backend (`extensions/acpx`), JARVIS and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses JARVIS `sessionKey`)
...
- JARVIS consumes ndjson events from `acpx --format json --json-strict`
