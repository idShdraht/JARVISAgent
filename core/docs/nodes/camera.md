...
JARVIS supports **camera capture** for agent workflows:
...
```bash
jarvis nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
jarvis nodes camera snap --node <id> --facing front
jarvis nodes camera clip --node <id> --duration 3000
jarvis nodes camera clip --node <id> --no-audio
```
...
```bash
jarvis nodes camera list --node <id>            # list camera ids
jarvis nodes camera snap --node <id>            # prints MEDIA:<path>
jarvis nodes camera snap --node <id> --max-width 1280
jarvis nodes camera snap --node <id> --delay-ms 2000
jarvis nodes camera snap --node <id> --device-id <id>
jarvis nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
jarvis nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
jarvis nodes camera clip --node <id> --device-id <id>
jarvis nodes camera clip --node <id> --no-audio
```

Notes:

- `jarvis nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
jarvis nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
