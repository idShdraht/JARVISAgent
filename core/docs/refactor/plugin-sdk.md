...
- Publish as `jarvis/plugin-sdk` (or export from core under `jarvis/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `JARVISPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: JARVISConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: JARVISConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: JARVISConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: JARVISConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: JARVISConfig): string;
  };
};
```
...
- Introduce `jarvis/plugin-sdk`.
- Add `api.runtime` to `JARVISPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `jarvisRuntime: ">=2026.2.0"`).
