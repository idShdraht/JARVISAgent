...
read_when:
  - You want to place an outbound voice call from JARVIS
...
Voice calls for JARVIS via a plugin. Supports outbound notifications and
...
- Use `jarvis voicecall ...` or the `voice_call` tool
...
```bash
jarvis plugins install ./extensions/voice-call
