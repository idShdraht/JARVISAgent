---
summary: "JARVIS on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up JARVIS on Oracle Cloud
  - Looking for low-cost VPS hosting for JARVIS
  - Want 24/7 JARVIS on a small server
title: "Oracle Cloud"
---

# JARVIS on Oracle Cloud (OCI)

## Goal

Run a persistent JARVIS Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for JARVIS (especially if you already have an OCI account), but it comes with tradeoffs:
...
sudo tailscale up --ssh --hostname=jarvis
```

This enables Tailscale SSH, so you can connect via `ssh jarvis` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@jarvis` (or use the Tailscale IP).
...
jarvis config set gateway.bind loopback

# Require auth for the Gateway + Control UI
jarvis config set gateway.auth.mode token
jarvis doctor --generate-gateway-token

# Expose over Tailscale Serve (HTTPS + tailnet access)
jarvis config set gateway.tailscale.mode serve
jarvis config set gateway.trustedProxies '["127.0.0.1"]'
...
```
https://jarvis.<tailnet-name>.ts.net/
```
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `jarvis security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.jarvis`
- **Security audit:** `jarvis security audit`
...
ssh -L 18789:127.0.0.1:18789 ubuntu@jarvis
```
...
# Re-authenticate
sudo tailscale up --ssh --hostname=jarvis --reset
```
...
- `~/.jarvis/workspace/` — workspace (SOUL.md, memory, artifacts)
