...
service via the external `jarvis` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.jarvis.gateway`
  (or `ai.jarvis.<profile>` when using `--profile`/`JARVIS_PROFILE`; legacy `com.jarvis.*` is supported).
