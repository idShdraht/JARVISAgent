...
- Same path: run the app from a fixed location (for JARVIS, `dist/JARVIS.app`).
...
```bash
sudo tccutil reset Accessibility ai.jarvis.mac
sudo tccutil reset ScreenCapture ai.jarvis.mac
...
Workaround: move files into the JARVIS workspace (`~/.jarvis/workspace`) if you want to avoid per-folder grants.
