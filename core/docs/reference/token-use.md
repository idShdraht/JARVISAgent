---
summary: "How JARVIS builds prompt context and reports token usage + costs"
read_when:
...
JARVIS tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

JARVIS assembles its own system prompt on every run. It includes:
...
For images, JARVIS downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from JARVIS session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `jarvis status --usage` and `jarvis channels list` show
...
`cacheWrite`. If pricing is missing, JARVIS shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. JARVIS can
...
Anthropic's 1M context window is currently beta-gated. JARVIS can inject the
...
JARVIS skips the `context-1m-*` beta header because Anthropic currently
