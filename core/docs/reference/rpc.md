...
JARVIS integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- JARVIS owns lifecycle when `channels.signal.autoStart=true`.
...
- JARVIS spawns `imsg rpc` as a child process (legacy iMessage integration).
