...
This document explains how JARVIS manages sessions end-to-end:
...
JARVIS is designed around a single **Gateway process** that owns session state.
...
JARVIS persists sessions in two layers:
...
In `mode: "warn"`, JARVIS reports potential evictions but does not mutate the store/files.
...
JARVIS intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (JARVIS consumes the events, but Pi decides when to compact).
...
JARVIS also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, JARVIS bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, JARVIS leaves it alone.
...
JARVIS supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- JARVIS strips/suppresses this in the delivery layer.

As of `2026.1.10`, JARVIS also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
JARVIS uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but JARVIS’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `jarvis status`.
