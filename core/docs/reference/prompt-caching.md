...
- With Anthropic API-key auth profiles, JARVIS seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, JARVIS injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
JARVIS exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.jarvis/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$JARVIS_STATE_DIR/logs/cache-trace.jsonl`
...
- `JARVIS_CACHE_TRACE=1` enables cache tracing.
- `JARVIS_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `JARVIS_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `JARVIS_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `JARVIS_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
