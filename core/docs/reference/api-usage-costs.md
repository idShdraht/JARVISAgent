...
JARVIS features that can generate provider usage or paid API calls.
...
- `jarvis status --usage` and `jarvis channels list` show provider **usage windows**
...
JARVIS can pick up credentials from:
...
`jarvis models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
