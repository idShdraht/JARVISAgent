...
summary: "Default JARVIS agent instructions and skills roster for the personal assistant setup"
read_when:
  - Starting a new JARVIS agent session
  - Enabling or auditing default skills
---

# AGENTS.md — JARVIS Personal Assistant (default)
...
- For browser-driven verification, use `jarvis browser` (tabs/status/screenshot) with the JARVIS-managed Chrome profile.
- For DOM inspection, use `jarvis browser eval|query|dom|snapshot` (and `--json`/`--out` when you need machine output).
- For interactions, use `jarvis browser click|type|hover|drag|select|upload|press|wait|navigate|back|evaluate|run` (click/type require snapshot refs; use `evaluate` for CSS selectors).
