...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects JARVIS to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
jarvis plugins install ./extensions/feishu
```
...
If you just installed JARVIS, run the wizard:
...
2. Configuring app credentials in JARVIS
...
1. You already ran `jarvis channels add` for Feishu
2. The gateway is running (`jarvis gateway status`)
...
```bash
jarvis pairing approve feishu <CODE>
```
...
2. Run `jarvis logs --follow` and look for `chat_id`
...
2. Run `jarvis logs --follow` and look for `open_id`
...
| `jarvis gateway restart` | Restart gateway service       |
| `jarvis logs --follow`   | Tail gateway logs             |
...
4. Check logs: `jarvis logs --follow`
...
5. Ensure the gateway is running: `jarvis gateway status`
6. Check logs: `jarvis logs --follow`
