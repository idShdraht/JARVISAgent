...
```bash
jarvis plugins install @jarvis/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
jarvis plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "JARVIS" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `jarvis pairing list nextcloud-talk`
  - `jarvis pairing approve nextcloud-talk <CODE>`
