---
summary: "Synology Chat webhook setup and JARVIS config"
read_when:
  - Setting up Synology Chat with JARVIS
...
```bash
jarvis plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your JARVIS gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in JARVIS.
...
- Pairing approvals work with:
  - `jarvis pairing list synology-chat`
  - `jarvis pairing approve synology-chat <CODE>`
...
```bash
jarvis message send --channel synology-chat --target 123456 --text "Hello from JARVIS"
jarvis message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
