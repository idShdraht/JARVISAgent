...
```bash
jarvis plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
JARVIS will offer the local install path automatically.
...
3. Configure JARVIS with those credentials.
...
- Talk to JARVIS via Teams DMs, group chats, or channels.
...
- On startup, JARVIS resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
Before configuring JARVIS, you need to create an Azure Bot resource.
...
```json
{
  ...
  "description": { "short": "JARVIS in Teams", "full": "JARVIS in Teams" },
...
JARVIS handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, JARVIS only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/JARVISShared/` folder in the configured SharePoint site's default document library.
...
```bash
jarvis message send --channel msteams \
...
jarvis message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
jarvis message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
jarvis message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
jarvis message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
