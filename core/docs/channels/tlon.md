...
Tlon is a decentralized messenger built on Urbit. JARVIS connects to your Urbit ship and can
...
```bash
jarvis plugins install ./extensions/tlon
```
...
By default, JARVIS blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `jarvis message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, JARVIS replies in-thread.
