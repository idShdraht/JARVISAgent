...
Matrix is an open, decentralized messaging protocol. JARVIS connects as a Matrix **user**
...
```bash
jarvis plugins install @jarvis/matrix
```

Local checkout (when running from a git repo):

```bash
jarvis plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `jarvis plugins install @jarvis/matrix`
   - From a local checkout: `jarvis plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: JARVIS calls the same
     login endpoint, stores the access token in `~/.jarvis/credentials/matrix/credentials.json`,
...
- On first connection, JARVIS requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  JARVIS logs a warning.
...
`~/.jarvis/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `jarvis pairing list matrix`
  - `jarvis pairing approve matrix <CODE>`
...
- On startup, JARVIS resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
