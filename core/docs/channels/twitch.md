...
read_when:
  - Setting up Twitch chat integration for JARVIS
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. JARVIS connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
jarvis plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `JARVIS_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "jarvis", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use JARVIS_TWITCH_ACCESS_TOKEN env var)
