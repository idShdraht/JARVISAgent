...
```bash
jarvis tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `jarvis tui --deliver`
...
- Check the Gateway logs: `jarvis logs --follow`.
- Confirm the agent can run: `jarvis status` and `jarvis models status`.
...
- No agents in picker: check `jarvis agents list` and your routing config.
