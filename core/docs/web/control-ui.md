...
If the page fails to load, start the Gateway first: `jarvis gateway`.
...
you revoke it with `jarvis devices revoke --device <id> --role <role>`. See
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. JARVIS
...
JARVIS **blocks** Control UI connections without device identity.
