...
Deploy JARVIS on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/jarvis/jarvis)
...
repository configures everything needed to run JARVIS:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: JARVIS_STATE_DIR
        value: /data/.jarvis
      - key: JARVIS_WORKSPACE_DIR
        value: /data/workspace
      - key: JARVIS_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/jarvis`.
...
If you use the original JARVIS repository, Render will not auto-deploy your JARVIS. To update it, run a manual Blueprint sync from the dashboard.
...
For JARVIS, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any JARVIS host.
