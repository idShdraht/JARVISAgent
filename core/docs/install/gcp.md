---
summary: "Run JARVIS Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want JARVIS running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# JARVIS on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent JARVIS Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "JARVIS 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
```bash
gcloud compute instances create jarvis-gateway \
  --zone=us-central1-a \
...
```bash
gcloud compute ssh jarvis-gateway --zone=us-central1-a
```
...
When binding to LAN (`JARVIS_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

```bash
docker compose run --rm jarvis-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
```
...
```bash
docker compose exec jarvis-gateway which gog
docker compose exec jarvis-gateway which goplaces
docker compose exec jarvis-gateway which wacli
```
...
```bash
gcloud compute ssh jarvis-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
```
...
```bash
# Stop the VM first
gcloud compute instances stop jarvis-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type jarvis-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start jarvis-gateway --zone=us-central1-a
```
