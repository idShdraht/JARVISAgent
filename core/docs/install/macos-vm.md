---
summary: "Run JARVIS in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want JARVIS isolated from your main macOS environment
...
# JARVIS on macOS VMs (Sandboxing)
...
Run JARVIS in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume).
...
5. SSH in, install JARVIS, configure channels
6. Done
...
The VM runs in the background. JARVIS's daemon keeps the gateway running.

To check status:

```bash
ssh youruser@192.168.64.X "jarvis status"
```
...
This is the killer feature of running on macOS. Use [BlueBubbles](https://bluebubbles.app) to add iMessage to JARVIS.
...
| VM IP not showing        | Wait for VM to fully boot, run `lume get jarvis` again                           |
| Lume command not found   | Add `~/.local/bin` to your PATH                                                    |
| WhatsApp QR not scanning | Ensure you're logged into the VM (not host) when running `jarvis channels login` |
