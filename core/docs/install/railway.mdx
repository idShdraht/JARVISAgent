...
Deploy JARVIS on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/jarvis` — Control UI
...
- `JARVIS_STATE_DIR=/data/.jarvis` (recommended)
- `JARVIS_WORKSPACE_DIR=/data/workspace` (recommended)
- `JARVIS_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your JARVIS state + workspace so you can migrate to another host without losing config or memory.
