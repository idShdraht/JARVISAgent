---
summary: "Use Venice AI privacy-focused models in JARVIS"
read_when:
  - You want privacy-focused inference in JARVIS
...
```bash
jarvis onboard --auth-choice venice-api-key
```
...
```bash
jarvis agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, JARVIS shows all available Venice models. Pick based on your needs:
...
```bash
jarvis models set venice/claude-opus-45
jarvis models set venice/llama-3.3-70b
```

List all available models:

```bash
jarvis models list | grep venice
```
...
JARVIS automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
jarvis agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
jarvis agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
jarvis agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
jarvis agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
jarvis agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `jarvis models list` to see currently available models. Some models may be temporarily offline.
