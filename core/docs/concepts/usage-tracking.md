...
- `/usage cost` in chats: local cost summary aggregated from JARVIS session logs.
- CLI: `jarvis status --usage` prints a full per-provider breakdown.
- CLI: `jarvis channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
