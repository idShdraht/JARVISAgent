---
summary: "Chrome extension: let JARVIS drive your existing Chrome tab"
read_when:
...
JARVIS then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the JARVIS release (npm package) as static files. There is no separate “build” step.

After upgrading JARVIS:

- Re-run `jarvis browser extension install` to refresh the installed files under your JARVIS state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

JARVIS ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want JARVIS to control.
...
- `ON`: attached; JARVIS can drive that tab.
...
Debugging: `jarvis sandbox explain`
...
`jarvis browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `jarvis browser extension install` first to copy the extension to a stable location under your JARVIS state directory.
