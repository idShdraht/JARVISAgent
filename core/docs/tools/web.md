...
If no `provider` is explicitly set, JARVIS auto-detects which provider to use based on available API keys, checking in this order:
...
3. Run `jarvis configure --section web` to store the key in config (recommended), or set `BRAVE_API_KEY` in your environment.
...
**Recommended:** run `jarvis configure --section web`. It stores the key in
`~/.jarvis/jarvis.json` under `tools.web.search.apiKey`.

**Environment alternative:** set `BRAVE_API_KEY` in the Gateway process
environment. For a gateway install, put it in `~/.jarvis/.env` (or your
service environment). See [Env vars](/help/faq#how-does-jarvis-load-environment-variables).
...
environment. For a gateway install, put it in `~/.jarvis/.env`.

If no base URL is set, JARVIS chooses a default based on the API key source:
...
For a gateway install, put it in `~/.jarvis/.env`.
