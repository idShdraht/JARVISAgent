...
read_when:
  ...
  - Wiring external systems into JARVIS
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your JARVIS config directory (typically `~/.jarvis/hooks/transforms`).
...
- `jarvis webhooks gmail setup` writes `hooks.gmail` config for `jarvis webhooks gmail run`.
