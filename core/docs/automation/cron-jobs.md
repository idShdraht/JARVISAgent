...
- Jobs persist under `~/.jarvis/cron/` so restarts don’t lose schedules.
...
jarvis cron list
jarvis cron run <job-id>
jarvis cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.jarvis/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `jarvis cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, JARVIS applies a
...
If `delivery` is omitted for isolated jobs, JARVIS defaults to `announce`.
...
- Job store: `~/.jarvis/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.jarvis/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
- For removed run sessions no longer referenced by the session store, JARVIS archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `jarvis cron runs` and adjust retention before logs become large
...
jarvis cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
jarvis cron edit <jobId> --agent ops
jarvis cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
jarvis cron run <jobId>
jarvis cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
jarvis cron edit <jobId> \
...
```bash
jarvis system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`jarvis system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `JARVIS_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- JARVIS applies exponential retry backoff for recurring jobs after consecutive errors:
