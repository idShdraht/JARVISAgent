...
jarvis system heartbeat last
jarvis logs --follow
jarvis config get agents.defaults.heartbeat
jarvis channels status --probe
```
...
```bash
jarvis config get agents.defaults.heartbeat.activeHours
jarvis config get agents.defaults.heartbeat.activeHours.timezone
jarvis config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
jarvis cron list
jarvis logs --follow
```
