...
struct JARVISMacCLI {
    static func main() async {
        ...
        default:
            fputs("jarvis-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      jarvis-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      jarvis-mac discover [--timeout <ms>] [--json] [--include-local]
      jarvis-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      jarvis-mac connect
      jarvis-mac connect --url ws://127.0.0.1:18789 --json
      jarvis-mac discover --timeout 3000 --json
      jarvis-mac wizard --mode local
    """)
}
