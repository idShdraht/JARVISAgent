...
enum CommandResolver {
    private static let projectRootDefaultsKey = "jarvis.gatewayProjectRootPath"
    private static let helperName = "jarvis"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let jarvisEntry = root.appendingPathComponent("jarvis.mjs").path
        if FileManager().isReadableFile(atPath: jarvisEntry) { return jarvisEntry }
        let binEntry = root.appendingPathComponent("bin/jarvis.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/jarvis")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let jarvisPaths = self.jarvisManagedPaths(home: home)
        if !jarvisPaths.isEmpty {
            extras.insert(contentsOf: jarvisPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + jarvisPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func jarvisManagedPaths(home: URL) -> [String] {
        let bases = [
    ...
    static func jarvisExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectJARVISExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("jarvis.mjs").path,
            root.appendingPathComponent("bin/jarvis.js").path,
        ]
    ...
    static func hasAnyJARVISInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.jarvisExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let jarvisPath = self.projectJARVISExecutable(projectRoot: root) {
            return [jarvisPath, subcommand] + extraArgs
        }
        if let jarvisPath = self.jarvisExecutable(searchPaths: searchPaths) {
            return [jarvisPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "jarvis", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            jarvis entrypoint missing (looked for dist/index.js or jarvis.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real jarvis CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/jarvis"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/jarvis.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/jarvis.mjs"
            node "$PRJ/jarvis.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/jarvis.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/jarvis.js"
            node "$PRJ/bin/jarvis.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent jarvis"
          pnpm --silent jarvis \(quotedArgs);
        else
          echo "jarvis CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? JARVISConfigFile.loadDict()
