...
private let webChatSwiftLogger = Logger(subsystem: "ai.jarvis", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: JARVISChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> JARVISChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(JARVISChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> JARVISChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                JARVISGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<JARVISChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any JARVISChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = JARVISChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: JARVISChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<JARVISChatView>) -> NSViewController
    {
