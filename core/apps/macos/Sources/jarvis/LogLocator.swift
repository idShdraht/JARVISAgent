...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["JARVIS_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/jarvis")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("jarvis-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("jarvis-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/jarvis/ (rolling or stdout), or nil if none exist.
