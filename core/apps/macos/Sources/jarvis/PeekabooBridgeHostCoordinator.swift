...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.jarvis", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: JARVISPeekabooBridgeServices?
    private static var jarvisSocketPath: String {
    ...
    private func startIfNeeded() async {
        ...
        let services = JARVISPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.jarvisSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.jarvisSocketPath, privacy: .public)")
    }
...
@MainActor
private final class JARVISPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.jarvis.peekaboo")
