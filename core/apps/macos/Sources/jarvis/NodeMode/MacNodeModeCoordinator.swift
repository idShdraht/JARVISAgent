...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.jarvis", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: JARVISNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [JARVISCapability.canvas.rawValue, JARVISCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(JARVISCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if JARVISLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(JARVISCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            JARVISSystemCommand.which.rawValue,
            JARVISSystemCommand.run.rawValue,
            JARVISSystemCommand.execApprovalsGet.rawValue,
            JARVISSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(JARVISCapability.camera.rawValue) {
        ...
        if capsSet.contains(JARVISCapability.location.rawValue) {
