...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.jarvis", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> JARVISSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: JARVISSessionPreviewEntry,
    ...
    private static func cache(payload: JARVISSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: JARVISChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [JARVISKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [JARVISKit.AnyCodable]) -> [JARVISChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(JARVISChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: JARVISChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: JARVISChatMessage) -> Bool {
    ...
    private static func toolNames(for message: JARVISChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: JARVISChatMessage) -> String? {
