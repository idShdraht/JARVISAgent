...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("jarvis").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing jarvis CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed jarvis \($0)." } ?? "Installed jarvis."
    ...
    private static func installScriptCommand(version: String, prefix: String) -> [String] {
        ...
        let script = """
        curl -fsSL https://jarvis.bot/install-cli.sh | \
