...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: JARVISProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.jarvis", category: "control")
    ...
    private static func bridgeToProtocolArgs(
        _ value: JARVISProtocol.AnyCodable?) -> [String: JARVISProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: JARVISProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: JARVISKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: JARVISProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("jarvis.control.heartbeat")
    static let controlAgentEvent = Notification.Name("jarvis.control.agent")
}
