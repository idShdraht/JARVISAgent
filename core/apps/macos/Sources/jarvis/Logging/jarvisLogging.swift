...
enum JARVISLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = JARVISOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = JARVISFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        JARVISLogging.bootstrapIfNeeded()
        let label = JARVISLogging.makeLabel(subsystem: subsystem, category: category)
...
struct JARVISOSLogHandler: LogHandler {
...
struct JARVISFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = JARVISLogging.parseLabel(self.label)
