...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.jarvis.mac`) even if the shipped
    ...
    {
        if environment["JARVIS_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "jarvis.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "jarvis.nixMode") { return true }
