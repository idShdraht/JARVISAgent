...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.jarvis/jarvis.json. Prefer JARVIS_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.jarvis/jarvis.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.jarvis/jarvis.json. Restart the gateway to apply."
        }
