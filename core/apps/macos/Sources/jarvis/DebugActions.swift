...
enum DebugActions {
    private static let verboseDefaultsKey = "jarvis.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "jarvis.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = JARVISPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "JARVIS", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = JARVISPaths.configURL
        guard
