...
private let deepLinkLogger = Logger(subsystem: "ai.jarvis", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "JARVIS is paused", message: "Unpause JARVIS to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run JARVIS agent?", message: body) else { return }
        }
