...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersJARVISBinary() async throws {
        ...
        let jarvisPath = tmp.appendingPathComponent("node_modules/.bin/jarvis")
        try self.makeExec(at: jarvisPath)

        let cmd = CommandResolver.jarvisCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([jarvisPath.path, "gateway"]))
    }
    ...
    @Test func fallsBackToNodeAndScript() async throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/jarvis.js")
        try self.makeExec(at: nodePath)
        try "#!/bin/sh\necho v22.0.0\n".write(to: nodePath, atomically: true, encoding: .utf8)
        try FileManager().setAttributes([.posixPermissions: 0o755], ofItemAtPath: nodePath.path)
        try self.makeExec(at: scriptPath)

        let cmd = CommandResolver.jarvisCommand(
            subcommand: "rpc",
            defaults: defaults,
            configRoot: [:],
    ...
    @Test func prefersJARVISBinaryOverPnpm() async throws {
        let defaults = self.makeDefaults()
        defaults.set(AppState.ConnectionMode.local.rawValue, forKey: connectionModeKey)
        ...
        let jarvisPath = binDir.appendingPathComponent("jarvis")
        ...
        #expect(cmd.prefix(2).elementsEqual([jarvisPath.path, "rpc"]))
    }

    @Test func usesJARVISBinaryWithoutNodeRuntime() async throws {
        let defaults = self.makeDefaults()
        defaults.set(AppState.ConnectionMode.local.rawValue, forKey: connectionModeKey)
    ...
    @Test func fallsBackToPnpm() async throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "jarvis", "rpc"]))
    }
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() async throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "jarvis", "health", "--json"]))
    ...
    @Test func buildsSSHCommandForRemoteMode() async throws {
        ...
        defaults.set("jarvis@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/jarvis", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "jarvis@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/jarvis'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("jarvis"))
    ...
    @Test func configRootLocalOverridesRemoteDefaults() async throws {
        ...
        #expect(cmd.first == jarvisPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
