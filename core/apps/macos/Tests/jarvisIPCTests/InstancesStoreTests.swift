...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: JARVISProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: JARVISProtocol.AnyCodable] = [
            "presence": .init([JARVISProtocol.AnyCodable(entry)]),
        ]
        let payload = JARVISProtocol.AnyCodable(payloadMap)
