...
- Canvas web content already supports live reload when loaded from Gateway `__jarvis__/canvas/` (see `docs/platforms/android.md`).
...
7) Canvas host is enabled and reachable from the device (do not run gateway with `JARVIS_SKIP_CANVAS_HOST=1`; startup logs should include `canvas host mounted at .../__jarvis__/`).
