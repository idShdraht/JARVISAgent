  ...
  AndroidView(
    ...
    factory = {
      WebView(context).apply {
        ...
        if (isDebuggable) {
          Log.d("JARVISWebView", "userAgent: ${settings.userAgentString}")
        }
        ...
        webViewClient =
          object : WebViewClient() {
            override fun onReceivedError(
              ...
              Log.e("JARVISWebView", "onReceivedError: ${error.errorCode} ${error.description} ${request.url}")
            }
            ...
            override fun onPageFinished(view: WebView, url: String?) {
              if (isDebuggable) {
                Log.d("JARVISWebView", "onPageFinished: $url")
              }
...
private class CanvasA2UIActionBridge(private val onMessage: (String) -> Unit) {
  ...
  companion object {
    const val interfaceName: String = "jarvisCanvasA2UIAction"
  }
}
