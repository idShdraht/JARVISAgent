...
private final class TestChatTransport: @unchecked Sendable, JARVISChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [JARVISChatHistoryPayload]
    private let sessionsResponses: [JARVISChatSessionsListResponse]

    private let stream: AsyncStream<JARVISChatTransportEvent>
    private let continuation: AsyncStream<JARVISChatTransportEvent>.Continuation

    init(
        historyResponses: [JARVISChatHistoryPayload],
        sessionsResponses: [JARVISChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<JARVISChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> JARVISChatHistoryPayload {
        ...
        return self.historyResponses.last ?? JARVISChatHistoryPayload(
        ...
        attachments _: [JARVISChatAttachmentPayload]) async throws -> JARVISChatSendResponse
    {
        ...
        return JARVISChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> JARVISChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? JARVISChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: JARVISChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = JARVISChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = JARVISChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { JARVISChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = JARVISChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = JARVISChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = JARVISChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { JARVISChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
