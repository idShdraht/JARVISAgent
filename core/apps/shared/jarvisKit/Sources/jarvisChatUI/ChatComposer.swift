...
@MainActor
struct JARVISChatComposer: View {
    @Bindable var viewModel: JARVISChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(JARVISChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(JARVISChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \JARVISPendingAttachment.id)
                { (att: JARVISPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            JARVISPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(JARVISChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(JARVISChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(JARVISChatTheme.subtleCard)
        .clipShape(Capsule())
    }
