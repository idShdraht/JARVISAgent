...
private let chatUILogger = Logger(subsystem: "ai.jarvis", category: "JARVISChatUI")

@MainActor
@Observable
public final class JARVISChatViewModel {
    public private(set) var messages: [JARVISChatMessage] = []
    ...
    public var attachments: [JARVISPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [JARVISChatPendingToolCall] = []
    public private(set) var sessions: [JARVISChatSessionEntry] = []
    private let transport: any JARVISChatTransport
    ...
    private var pendingToolCallsById: [String: JARVISChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any JARVISChatTransport) {
    ...
    public var sessionChoices: [JARVISChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [JARVISChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: JARVISPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [JARVISChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: JARVISChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: JARVISChatMessage) -> JARVISChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> JARVISChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return JARVISChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: JARVISChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [JARVISChatMessage],
        incoming: [JARVISChatMessage]) -> [JARVISChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [JARVISChatMessage]) -> [JARVISChatMessage] {
        var result: [JARVISChatMessage] = []
    ...
    private static func dedupeKey(for message: JARVISChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [JARVISChatMessageContent] = [
            JARVISChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> JARVISChatAttachmentPayload in
            JARVISChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> JARVISChatSessionEntry {
        JARVISChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: JARVISChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: JARVISChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: JARVISAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = JARVISChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> JARVISPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
