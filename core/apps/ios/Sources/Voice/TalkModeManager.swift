...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__JARVIS_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<JARVISTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.jarvis", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = JARVISTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> JARVISTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return JARVISTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> JARVISTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> JARVISTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return JARVISTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> JARVISTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = JARVISTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: JARVISTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: JARVISAgentEventPayload.self
            ) else {
                continue
            }
