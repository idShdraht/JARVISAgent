...
final class ContactsService: ContactsServicing {
    ...
    func search(params: JARVISContactsSearchParams) async throws -> JARVISContactsSearchPayload {
        ...
        return JARVISContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: JARVISContactsAddParams) async throws -> JARVISContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return JARVISContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return JARVISContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> JARVISContactPayload {
        JARVISContactPayload(
