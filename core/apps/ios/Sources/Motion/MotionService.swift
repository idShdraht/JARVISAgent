...
final class MotionService: MotionServicing {
    func activities(params: JARVISMotionActivityParams) async throws -> JARVISMotionActivityPayload {
        ...
        let mapped: [JARVISMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return JARVISMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: JARVISPedometerParams) async throws -> JARVISPedometerPayload {
        ...
        let payload: JARVISPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = JARVISPedometerPayload(
