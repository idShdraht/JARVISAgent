...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> JARVISDeviceStatusPayload {
        ...
        return JARVISDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> JARVISDeviceInfoPayload {
        ...
        return JARVISDeviceInfoPayload(
    ...
    private func batteryStatus() -> JARVISBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: JARVISBatteryState = switch device.batteryState {
        ...
        return JARVISBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> JARVISThermalStatusPayload {
        let state: JARVISThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return JARVISThermalStatusPayload(state: state)
    }

    private func storageStatus() -> JARVISStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return JARVISStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
