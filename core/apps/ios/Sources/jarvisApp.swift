...
@MainActor
final class JARVISAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.jarvis.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.jarvis.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.jarvis.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "jarvis.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "jarvis.watch.promptId"
    static let sessionKeyKey = "jarvis.watch.sessionKey"
    static let actionPrimaryIDKey = "jarvis.watch.action.primary.id"
    static let actionPrimaryLabelKey = "jarvis.watch.action.primary.label"
    static let actionSecondaryIDKey = "jarvis.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "jarvis.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "jarvis.watch.action.primary"
    static let actionSecondaryIdentifier = "jarvis.watch.action.secondary"
    static let actionIdentifierPrefix = "jarvis.watch.action."
    static let actionIDKeyPrefix = "jarvis.watch.action.id."
    static let actionLabelKeyPrefix = "jarvis.watch.action.label."
    static let categoryPrefix = "jarvis.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: JARVISWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> JARVISWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return JARVISWatchAction(id: id, label: label, style: action.style)
        }
    ...
    private static func categoryActions(_ actions: [JARVISWatchAction]) -> [UNNotificationAction] {
...
extension JARVISApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("JARVIS: installing uncaught exception handler")
