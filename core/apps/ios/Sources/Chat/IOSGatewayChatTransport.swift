...
struct IOSGatewayChatTransport: JARVISChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.jarvis", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> JARVISChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(JARVISChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> JARVISChatHistoryPayload {
        ...
        return try JSONDecoder().decode(JARVISChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [JARVISChatAttachmentPayload]) async throws -> JARVISChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [JARVISChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(JARVISChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(JARVISGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: JARVISGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: JARVISChatEventPayload.self)
                        {
