...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.jarvis.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to JARVIS", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to JARVIS gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to JARVIS.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "JARVISShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "JARVIS is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "JARVIS Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("jarvis-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
