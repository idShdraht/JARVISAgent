...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("jarvis-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("jarvis-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("jarvis-lid-a-", (first) => {
      withTempDirSync("jarvis-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.jarvis when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "jarvis-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers JARVIS_HOME over HOME", () => {
    vi.stubEnv("JARVIS_HOME", "/srv/jarvis-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/jarvis-home"));
...
describe("shortenHomePath", () => {
  it("uses $JARVIS_HOME prefix when JARVIS_HOME is set", () => {
    vi.stubEnv("JARVIS_HOME", "/srv/jarvis-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/jarvis-home")}/.jarvis/jarvis.json`)).toBe(
      "$JARVIS_HOME/.jarvis/jarvis.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $JARVIS_HOME replacement when JARVIS_HOME is set", () => {
    vi.stubEnv("JARVIS_HOME", "/srv/jarvis-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/jarvis-home")}/.jarvis/jarvis.json`),
    ).toBe("config: $JARVIS_HOME/.jarvis/jarvis.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/jarvis")).toBe(path.resolve(os.homedir(), "jarvis"));
  });
  ...
  it("prefers JARVIS_HOME for tilde expansion", () => {
    vi.stubEnv("JARVIS_HOME", "/srv/jarvis-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/jarvis")).toBe(path.resolve("/srv/jarvis-home", "jarvis"));
