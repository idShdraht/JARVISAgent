...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "JARVIS_STATE_DIR",
  "JARVIS_CONFIG_PATH",
  "JARVIS_SKIP_BROWSER_CONTROL_SERVER",
  "JARVIS_SKIP_GMAIL_WATCHER",
  "JARVIS_SKIP_CANVAS_HOST",
  "JARVIS_BUNDLED_PLUGINS_DIR",
  "JARVIS_SKIP_CHANNELS",
  "JARVIS_SKIP_PROVIDERS",
  "JARVIS_SKIP_CRON",
  "JARVIS_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.JARVIS_STATE_DIR = path.join(tempHome, ".jarvis");
  delete process.env.JARVIS_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.JARVIS_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.JARVIS_SKIP_GMAIL_WATCHER = "1";
  process.env.JARVIS_SKIP_CANVAS_HOST = "1";
  process.env.JARVIS_SKIP_CHANNELS = "1";
  process.env.JARVIS_SKIP_PROVIDERS = "1";
  process.env.JARVIS_SKIP_CRON = "1";
  process.env.JARVIS_TEST_MINIMAL_GATEWAY = "1";
  process.env.JARVIS_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "jarvis-test-no-bundled-extensions")
    : "jarvis-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "jarvis-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".jarvis-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__jarvisTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__jarvisTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["JARVIS_GATEWAY_TOKEN"]);
  const prev = process.env.JARVIS_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.JARVIS_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.JARVIS_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
