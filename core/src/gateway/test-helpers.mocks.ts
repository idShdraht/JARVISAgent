...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `jarvis-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "jarvis-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadJARVISPlugins: () => pluginRegistryState.registry,
  };
});
