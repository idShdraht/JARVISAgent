...
export function resolveHooksConfig(cfg: JARVISConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: JARVISConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-jarvis-token"] === "string"
      ? req.headers["x-jarvis-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
