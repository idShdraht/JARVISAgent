...
const { createJARVISTools } = await import("../agents/jarvis-tools.js");
...
type SessionSendTool = ReturnType<typeof createJARVISTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createJARVISTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["JARVIS_GATEWAY_PORT", "JARVIS_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.JARVIS_GATEWAY_PORT = String(gatewayPort);
  process.env.JARVIS_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("JARVIS_CONFIG_PATH missing in gateway test environment");
      }
