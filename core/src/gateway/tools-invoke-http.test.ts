...
let lastCreateJARVISToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/jarvis-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateJARVISToolsContext?.agentTo,
          agentThreadId: lastCreateJARVISToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createJARVISTools: (ctx: Record<string, unknown>) => {
      lastCreateJARVISToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateJARVISToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-jarvis-message-to": "channel:24514",
        "x-jarvis-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
