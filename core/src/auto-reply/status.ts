...
type AgentDefaults = NonNullable<NonNullable<JARVISConfig["agents"]>["defaults"]>;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🤖 JARVIS ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: JARVISConfig): string {
  const lines = ["ℹ️ Help", ""];
