...
import { scheduleGatewaySigusr1Restart, triggerJARVISRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting JARVIS in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerJARVISRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting JARVIS via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
