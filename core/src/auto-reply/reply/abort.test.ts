...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop jarvis!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@jarvis_bot", { botUsername: "jarvis_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@jarvis_bot", { botUsername: "jarvis_bot" })).toBe(true);
