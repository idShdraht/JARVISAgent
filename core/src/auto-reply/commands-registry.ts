...
export function isCommandEnabled(cfg: JARVISConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: JARVISConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as JARVISConfig),
...
export function getCommandDetection(_cfg?: JARVISConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: JARVISConfig) {
