...
import { discoverJARVISPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: JARVISPluginDefinition;
  register?: JARVISPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as JARVISPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as JARVISPluginDefinition;
...
export function loadJARVISPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverJARVISPlugins({
    workspaceDir: options.workspaceDir,
        ...
        ? {
            alias: {
              ...
              ...(pluginSdkAccountIdAlias
                ? { "jarvis/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: JARVISPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as JARVISPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
