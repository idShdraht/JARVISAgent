...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "jarvis", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "jarvis", version: "2.3.4" });
  ...
  it("prefers JARVIS_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        JARVIS_VERSION: "9.9.9",
        JARVIS_SERVICE_VERSION: "2.2.2",
