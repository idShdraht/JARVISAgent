...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "jarvis-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "jarvis-bundled-skills-test-"));
    process.env.JARVIS_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
