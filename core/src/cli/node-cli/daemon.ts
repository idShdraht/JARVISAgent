...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("jarvis node install"), formatCliCommand("jarvis node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("jarvis node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    JARVIS_LOG_PREFIX: baseEnv.JARVIS_LOG_PREFIX ?? "node",
