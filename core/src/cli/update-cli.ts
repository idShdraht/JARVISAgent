...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update JARVIS and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["jarvis update", "Update a source checkout (git)"],
        ["jarvis update --channel beta", "Switch to beta channel (git + npm)"],
        ["jarvis update --channel dev", "Switch to dev channel (git + npm)"],
        ["jarvis update --tag beta", "One-off update to a dist-tag or version"],
        ["jarvis update --dry-run", "Preview actions without changing anything"],
        ["jarvis update --no-restart", "Update without restarting the service"],
...
${theme.heading("Switch channels:")}
  ...
  - Run jarvis update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.jarvis.ai/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["jarvis update status", "Show channel + version status."],
          ["jarvis update status --json", "JSON output."],
          ["jarvis update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
