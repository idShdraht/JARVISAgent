...
const STEP_LABELS: Record<string, string> = {
  ...
  "jarvis doctor entry": "Checking doctor entrypoint",
  "jarvis doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g jarvis@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g jarvis@latest --omit=optional");
  }
