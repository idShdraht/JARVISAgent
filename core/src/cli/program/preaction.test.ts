...
beforeEach(() => {
  ...
  originalHideBanner = process.env.JARVIS_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.JARVIS_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.JARVIS_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("jarvis");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "jarvis", "status", "--debug"],
    });
  ...
  it("loads plugin registry for configure command", async () => {
    await runCommand({
      parseArgv: ["configure"],
      processArgv: ["node", "jarvis", "configure"],
    });
  ...
  it("loads plugin registry for onboard command", async () => {
    await runCommand({
      parseArgv: ["onboard"],
      processArgv: ["node", "jarvis", "onboard"],
    });
  ...
  it("loads plugin registry for agents command", async () => {
    await runCommand({
      parseArgv: ["agents"],
      processArgv: ["node", "jarvis", "agents"],
    });
  ...
  it("skips config guard for doctor, completion, and secrets commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "jarvis", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "jarvis", "completion"],
    });
    await runCommand({
      parseArgv: ["secrets"],
      processArgv: ["node", "jarvis", "secrets"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "jarvis", "--version"],
    });
  ...
  it("hides banner when JARVIS_HIDE_BANNER is truthy", async () => {
    process.env.JARVIS_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "jarvis", "status"],
    });
