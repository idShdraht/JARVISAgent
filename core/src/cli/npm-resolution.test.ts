...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@jarvis/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@jarvis/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@jarvis/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@jarvis/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @jarvis/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@jarvis/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@jarvis/plugin-alpha@1.2.3",
        installPath: "/tmp/jarvis/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@jarvis/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @jarvis/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@jarvis/plugin-alpha@latest",
      true,
      "/tmp/jarvis/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@jarvis/plugin-alpha@latest",
      installPath: "/tmp/jarvis/extensions/alpha",
      version: "1.2.3",
