...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.jarvis.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + JARVIS hooks")
    ...
    .option("--hook-url <url>", "JARVIS hook URL")
    .option("--hook-token <token>", "JARVIS hook token")
