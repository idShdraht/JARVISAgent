...
import type { JARVISConfig, ConfigFileSnapshot } from "../config/types.jarvis.js";
...
const { resolveJARVISPackageRoot } = await import("../infra/jarvis-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/jarvis-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as JARVISConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/jarvis-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveJARVISPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("jarvis-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveJARVISPackageRoot).mockClear();
    ...
    vi.mocked(resolveJARVISPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("jarvis");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/jarvis-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "jarvis-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("JARVIS update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as JARVISConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveJARVISPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("jarvis-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ JARVIS_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.JARVIS_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("jarvis-update-wizard");
    await withEnvAsync({ JARVIS_GIT_DIR: tempDir }, async () => {
      setTty(true);
