...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.jarvis${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  const stateDir = env.JARVIS_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.JARVIS_STATE_DIR?.trim()) {
    env.JARVIS_STATE_DIR = stateDir;
  }

  if (!env.JARVIS_CONFIG_PATH?.trim()) {
    env.JARVIS_CONFIG_PATH = path.join(stateDir, "jarvis.json");
  }

  if (profile === "dev" && !env.JARVIS_GATEWAY_PORT?.trim()) {
    env.JARVIS_GATEWAY_PORT = "19001";
  }
}
