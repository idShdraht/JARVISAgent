...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    JARVIS_STATE_DIR: "/tmp/jarvis-daemon",
    JARVIS_CONFIG_PATH: "/tmp/jarvis-daemon/jarvis.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.JARVIS_STATE_DIR ?? "/tmp/jarvis-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.JARVIS_CONFIG_PATH ?? `${stateDir}/jarvis.json`;
});

vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/jarvis-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.JARVIS_STATE_DIR = "/tmp/jarvis-cli";
    process.env.JARVIS_CONFIG_PATH = "/tmp/jarvis-cli/jarvis.json";
