...
const TEMP_MEDIA_PREFIX = "jarvis-media-";
...
function createGroqAudioConfig(): JARVISConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): JARVISConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): JARVISConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: JARVISConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredJARVISTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        JARVIS_AGENT_DIR: isolatedAgentDir,
