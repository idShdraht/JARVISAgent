...
function resolveGatewayAuthToken(cfg: JARVISConfig, env: NodeJS.ProcessEnv): string | undefined {
  const configToken = cfg.gateway?.auth?.token?.trim();
  if (configToken) {
    return configToken;
  }
  const envToken = env.JARVIS_GATEWAY_TOKEN ?? env.CLAWDBOT_GATEWAY_TOKEN;
