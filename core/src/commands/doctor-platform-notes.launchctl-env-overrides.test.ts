...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "JARVIS_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("JARVIS_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv JARVIS_GATEWAY_TOKEN");
    expect(message).not.toContain("JARVIS_GATEWAY_PASSWORD");
  });
