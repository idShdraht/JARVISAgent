 ...
 * Runs as part of `jarvis doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("jarvis config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("jarvis memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("jarvis configure --section model")}`,
        `- To disable: ${formatCliCommand("jarvis config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("jarvis memory status --deep")}`,
      ].join("\n"),
      "Memory search",
