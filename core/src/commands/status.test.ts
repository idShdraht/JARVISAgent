...
beforeAll(() => {
  envSnapshot = captureEnv(["JARVIS_PROFILE"]);
  process.env.JARVIS_PROFILE = "isolated";
});
...
vi.mock("../infra/jarvis-root.js", () => ({
  resolveJARVISPackageRoot: vi.fn().mockResolvedValue("/tmp/jarvis"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/jarvis/pnpm-lock.yaml",
      markerPath: "/tmp/jarvis/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.jarvis.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.jarvis.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("JARVIS status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("jarvis status --all") ||
          l.includes("jarvis --profile isolated status --all") ||
          l.includes("jarvis status --all") ||
          l.includes("jarvis --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.JARVIS_GATEWAY_TOKEN = "abcd1234";
    try {
