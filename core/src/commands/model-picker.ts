...
type PromptDefaultModelResult = { model?: string; config?: JARVISConfig };
...
function resolveConfiguredModelRaw(cfg: JARVISConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: JARVISConfig): string[] {
...
export function applyPrimaryModel(cfg: JARVISConfig, model: string): JARVISConfig {
...
export function applyModelAllowlist(cfg: JARVISConfig, models: string[]): JARVISConfig {
  const defaults = cfg.agents?.defaults;
