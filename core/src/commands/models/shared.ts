...
export async function loadValidConfigOrThrow(): Promise<JARVISConfig> {
...
export async function updateConfig(
  mutator: (cfg: JARVISConfig) => JARVISConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: JARVISConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: JARVISConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("jarvis agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
