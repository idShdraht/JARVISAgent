...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveJARVISAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("jarvis models auth setup-token")}\` or \`${formatCliCommand("jarvis configure")}\`.`
          : `Run \`${formatCliCommand("jarvis configure")}\` or set an API key env var.`;
