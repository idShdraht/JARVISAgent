...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: JARVISConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: JARVISConfig) => { changed: boolean; next: JARVISConfig };
  defaultModel: string;
  overrideConfig: JARVISConfig;
  alreadyDefaultConfig: JARVISConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: JARVISConfig) => config,
      applyDefaultConfig: (config: JARVISConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: JARVISConfig = { agents: { defaults: {} } };
