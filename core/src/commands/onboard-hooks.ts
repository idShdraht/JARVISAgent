...
export async function setupInternalHooks(
  ...
  await prompter.note(
    [
      ...
      "",
      "Learn more: https://docs.jarvis.ai/automation/hooks",
    ].join("\n"),
    "Hooks",
  );
  ...
  await prompter.note(
    [
      ...
      `  ${formatCliCommand("jarvis hooks list")}`,
      `  ${formatCliCommand("jarvis hooks enable <name>")}`,
      `  ${formatCliCommand("jarvis hooks disable <name>")}`,
    ].join("\n"),
    "Hooks Configured",
  );
