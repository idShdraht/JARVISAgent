...
function resolveAgentName(cfg: JARVISConfig, agentId: string) {
...
function resolveAgentModel(cfg: JARVISConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: JARVISConfig): AgentSummary[] {
