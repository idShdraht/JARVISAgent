...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.JARVIS_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.JARVIS_GATEWAY_PASSWORD;
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "JARVIS update wizard" : "JARVIS configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: JARVISConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("jarvis doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.JARVIS_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.JARVIS_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.JARVIS_GATEWAY_TOKEN;
