...
type EnvSnapshot = {
  HOME?: string;
  JARVIS_HOME?: string;
  JARVIS_STATE_DIR?: string;
  JARVIS_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    JARVIS_HOME: process.env.JARVIS_HOME,
    JARVIS_STATE_DIR: process.env.JARVIS_STATE_DIR,
    JARVIS_OAUTH_DIR: process.env.JARVIS_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: JARVISConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: JARVISConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "jarvis-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.JARVIS_HOME = tempHome;
    process.env.JARVIS_STATE_DIR = path.join(tempHome, ".jarvis");
    delete process.env.JARVIS_OAUTH_DIR;
    fs.mkdirSync(process.env.JARVIS_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when JARVIS_OAUTH_DIR is explicitly configured", async () => {
    process.env.JARVIS_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: JARVISConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints jarvis-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: JARVISConfig = {};
    setupSessionState(cfg, process.env, process.env.HOME ?? "");
    ...
    expect(text).toMatch(/jarvis sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/jarvis sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /jarvis sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
