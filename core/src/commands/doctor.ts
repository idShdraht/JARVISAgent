...
function resolveMode(cfg: JARVISConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveJARVISPackageRoot({
  ...
  let cfg: JARVISConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("jarvis configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("jarvis config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("jarvis setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("jarvis doctor --fix")}" to apply changes.`);
  }
