...
export function normalizeCompatibilityConfigValues(cfg: JARVISConfig): {
  config: JARVISConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: JARVISConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as JARVISConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as JARVISConfig["browser"],
    };
    changes.push(
