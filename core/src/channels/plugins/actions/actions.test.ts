...
function telegramCfg(): JARVISConfig {
  return { channels: { telegram: { botToken: "tok" } } } as JARVISConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: JARVISConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as JARVISConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as JARVISConfig;
...
function createSignalAccountOverrideCfg(): JARVISConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as JARVISConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as JARVISConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as JARVISConfig,
        accountId: undefined,
        params: {
