...
function cfg(accounts?: Record<string, unknown> | null): JARVISConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as JARVISConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as JARVISConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as JARVISConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as JARVISConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as JARVISConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as JARVISConfig)).toBe("default");
    });
  });
});
