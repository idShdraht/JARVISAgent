...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `jarvis-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      JARVIS_LAUNCHD_LABEL: `ai.jarvis.launchd-int-${testId}`,
      JARVIS_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
