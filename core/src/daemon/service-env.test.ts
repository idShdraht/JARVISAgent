...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.JARVIS_GATEWAY_PORT).toBe("18789");
    expect(env.JARVIS_GATEWAY_TOKEN).toBe("secret");
    expect(env.JARVIS_SERVICE_MARKER).toBe("jarvis");
    expect(env.JARVIS_SERVICE_KIND).toBe("gateway");
    expect(typeof env.JARVIS_SERVICE_VERSION).toBe("string");
    expect(env.JARVIS_SYSTEMD_UNIT).toBe("jarvis-gateway.service");
    if (process.platform === "darwin") {
      expect(env.JARVIS_LAUNCHD_LABEL).toBe("ai.jarvis.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", JARVIS_PROFILE: "work" },
      port: 18789,
    });
    expect(env.JARVIS_SYSTEMD_UNIT).toBe("jarvis-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.JARVIS_LAUNCHD_LABEL).toBe("ai.jarvis.work");
    }
  });
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".jarvis"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", JARVIS_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".jarvis-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", JARVIS_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".jarvis"));
  });

  it("uses JARVIS_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", JARVIS_STATE_DIR: "/var/lib/jarvis" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/jarvis"));
  });

  it("expands ~ in JARVIS_STATE_DIR", () => {
    const env = { HOME: "/Users/test", JARVIS_STATE_DIR: "~/jarvis-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/jarvis-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { JARVIS_STATE_DIR: "C:\\State\\jarvis" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\jarvis");
  });
});
