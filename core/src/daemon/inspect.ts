...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "jarvis" | "clawdbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["jarvis", "clawdbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["jarvis_service_marker"];
  const kindKeys = ["jarvis_service_kind"];
...
function isJARVISGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.jarvis.");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    if (marker === "jarvis" && isJARVISGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "jarvis" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (marker === "jarvis" && isJARVISGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
