...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".jarvis");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.JARVIS_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$JARVIS_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via JARVIS_STATE_DIR.
