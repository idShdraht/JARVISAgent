...
export const DEFAULT_LOG_DIR = resolvePreferredJARVISTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "jarvis.log"); // legacy single-file path
...
function resolveSettings(): ResolvedSettings {
  let cfg: JARVISConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => JARVISConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.JARVIS_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[jarvis] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
