...
const testLogPath = path.join(os.tmpdir(), "jarvis-test-env-log-level.log");
const defaultMaxFileBytes = 500 * 1024 * 1024;

describe("JARVIS_LOG_LEVEL", () => {
  let originalEnv: string | undefined;

  beforeEach(() => {
    originalEnv = process.env.JARVIS_LOG_LEVEL;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.JARVIS_LOG_LEVEL = originalEnv;
    }
    loggingState.invalidEnvLogLevelValue = null;
    resetLogger();
    setLoggerOverride(null);
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.JARVIS_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("JARVIS_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid JARVIS_LOG_LEVEL="nope"');
  });
});
