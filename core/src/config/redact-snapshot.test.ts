...
import type { ConfigFileSnapshot } from "./types.jarvis.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.jarvis/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/jarvis/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/jarvis/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/jarvis/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/jarvis/nickserv-password.txt");
  ...
  it("contract-covers dynamic catchall/record paths for redact+restore", () => {
    const hints = mapSensitivePaths(JARVISSchema, "", {});
    const snapshot = makeSnapshot({
      env: {
