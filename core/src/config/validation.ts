...
import type { JARVISConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: JARVISConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: JARVISConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = JARVISSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as JARVISConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as JARVISConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as JARVISConfig,
  };
}
