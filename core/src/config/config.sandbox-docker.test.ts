...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("jarvis-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "jarvis-sandbox-browser-global" },
      agentBrowser: { network: "jarvis-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("jarvis-sandbox-browser-agent");
  });
