...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ JARVIS_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-jarvis",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-jarvis#general");
  });
  ...
  it("derives session transcripts dir from JARVIS_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { JARVIS_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const stateDir = path.resolve("/home/user/.jarvis");
  ...
  it("resolves cross-agent paths when JARVIS_STATE_DIR differs from stored paths", () => {
  ...
  it("falls back to derived transcript path when sessionFile is outside agent sessions directories", () => {
    withStateDir(path.resolve("/home/user/.jarvis"), () => {
      const sessionFile = resolveSessionFilePath(
        "sess-1",
