...
function isWhatsAppConfigured(cfg: JARVISConfig): boolean {
...
function isGenericChannelConfigured(cfg: JARVISConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: JARVISConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: JARVISConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: JARVISConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: JARVISConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: JARVISConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: JARVISConfig, pluginId: string): JARVISConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
