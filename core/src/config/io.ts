...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  async function writeConfigFile(cfg: JARVISConfig, options: ConfigWriteOptions = {}) {
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.JARVIS_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.JARVIS_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.JARVIS_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.JARVIS_WATCH_SESSION === "string" &&
        deps.env.JARVIS_WATCH_SESSION.trim().length > 0
          ? deps.env.JARVIS_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.JARVIS_WATCH_COMMAND === "string" &&
        deps.env.JARVIS_WATCH_COMMAND.trim().length > 0
          ? deps.env.JARVIS_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `JARVIS_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.JARVIS_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.JARVIS_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
