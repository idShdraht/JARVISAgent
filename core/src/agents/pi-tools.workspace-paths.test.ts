...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("jarvis-ws-", async (workspaceDir) => {
      await withTempDir("jarvis-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createJARVISCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("jarvis-ws-", async (workspaceDir) => {
      await withTempDir("jarvis-override-", async (overrideDir) => {
        const tools = createJARVISCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("jarvis-ws-", async (workspaceDir) => {
      const cfg: JARVISConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createJARVISCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-jarvis.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("jarvis-sandbox-", async (sandboxDir) => {
      await withTempDir("jarvis-workspace-", async (workspaceDir) => {
        ...
        const tools = createJARVISCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
