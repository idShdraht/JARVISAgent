...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const jarvisTmpDir = path.resolve(resolvePreferredJARVISTmpDir());
  if (!isPathInside(jarvisTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: jarvisTmpDir });
  return resolved;
}
