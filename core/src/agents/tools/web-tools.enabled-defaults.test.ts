...
describe("web_search kimi provider", () => {
  ...
  it("runs the Kimi web_search tool flow and echoes tool results", async () => {
      ...
      if (idx === 1) {
        return new Response(
          JSON.stringify({
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        name: "$web_search",
                        arguments: JSON.stringify({ q: "jarvis" }),
                      },
                    },
                  ],
                },
            ...
            search_results: [
              { title: "JARVIS", url: "https://jarvis.ai/docs", content: "docs" },
            ],
          }),
    ...
    expect(JSON.parse(toolMessage?.content ?? "{}")).toMatchObject({
      search_results: [{ url: "https://jarvis.ai/docs" }],
    });
    ...
    expect(details.citations).toEqual(["https://jarvis.ai/docs"]);
