...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    jarvis: process.env.JARVIS_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.jarvis === undefined) {
      delete process.env.JARVIS_GATEWAY_TOKEN;
    } else {
      process.env.JARVIS_GATEWAY_TOKEN = envSnapshot.jarvis;
    }
  ...
  it("uses JARVIS_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.JARVIS_GATEWAY_TOKEN = "local-env-token";
