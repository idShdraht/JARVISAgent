...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): JARVISConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as JARVISConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): JARVISConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as JARVISConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: JARVISConfig, accountId = "media") {
    await handleTelegramAction(
