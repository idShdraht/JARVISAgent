 ...
 * A common mistake — caused by the similarity with the `jarvis.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (jarvis.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.JARVIS_AUTH_STORE_READONLY === "1";
