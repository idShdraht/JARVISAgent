...
const GOOGLE_LIVE_SINGLE_KEY = "JARVIS_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "JARVIS_LIVE_ANTHROPIC_KEY",
    listVar: "JARVIS_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "JARVIS_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `JARVIS_LIVE_${base}_KEY`;
