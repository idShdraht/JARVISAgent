...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "jarvis.sandbox=1");
  args.push("--label", `jarvis.sessionKey=${params.scopeKey}`);
  args.push("--label", `jarvis.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `jarvis.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "jarvis.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`jarvis sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`jarvis sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("jarvis sandbox recreate --all");
}
