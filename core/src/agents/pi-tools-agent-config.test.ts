...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: JARVISConfig) {
    return createJARVISCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<JARVISConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<JARVISConfig["agents"]>["list"]>[number]["tools"];
  }): JARVISConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createJARVISCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createJARVISCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createJARVISCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createJARVISCodingTools({
      config: cfg,
    ...
    const bobTools = createJARVISCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createJARVISCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createJARVISCodingTools({
      config: cfg,
