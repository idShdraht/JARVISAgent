...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["JARVIS_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-subagent-"));
  ...
  it("uses isolated temp state when JARVIS_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "jarvis-test-state"));
  });
});
