...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; JARVIS handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    ...
    "When diagnosing issues, run `jarvis status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running JARVIS process",
    agents_list: acpEnabled
      ? 'List JARVIS agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List JARVIS agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside JARVIS.";
  }

  const lines = [
    "You are a personal assistant running inside JARVIS.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control JARVIS's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "",
    ...safetySection,
    "## JARVIS CLI Quick Reference",
    "JARVIS is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- jarvis gateway status",
    "- jarvis gateway start",
    "- jarvis gateway stop",
    "- jarvis gateway restart",
    "If unsure, ask the user to run `jarvis help` (or `jarvis gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## JARVIS Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, JARVIS pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by JARVIS and included below in Project Context.",
    "",
