...
async function withOutsideHardlinkInJARVISTmp<T>(
  params: {
    jarvisTmpDir: string;
  ...
  const hardlinkPath = path.join(params.jarvisTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.jarvisTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.jarvisTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.jarvisTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const jarvisTmpDir = resolvePreferredJARVISTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred JARVIS tmp root",
      media: path.join(jarvisTmpDir, "image.png"),
      expected: path.join(jarvisTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred JARVIS tmp root",
      media: pathToFileURL(path.join(jarvisTmpDir, "photo.png")).href,
      expected: path.join(jarvisTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred JARVIS tmp root",
      media: path.join(jarvisTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(jarvisTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(jarvisTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked JARVIS tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(jarvisTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(jarvisTmpDir, { recursive: true });
      const symlinkPath = path.join(jarvisTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked JARVIS tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInJARVISTmp(
      {
        jarvisTmpDir,
  ...
  it("rejects symlinked JARVIS tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInJARVISTmp(
