...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: JARVISConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: JARVISConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: JARVISConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: JARVISConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: JARVISConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: JARVISConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/jarvis");
  });

  it("uses JARVIS_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "jarvis-home");
    vi.stubEnv("JARVIS_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as JARVISConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".jarvis", "workspace"));
  });

  it("uses JARVIS_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "jarvis-home");
    vi.stubEnv("JARVIS_HOME", home);
    // Clear state dir so it falls back to JARVIS_HOME
    vi.stubEnv("JARVIS_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as JARVISConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".jarvis", "agents", "main", "agent"));
  });
});
