...
import {
  ...
  normalizeStringList,
  parseJARVISManifestInstallBase,
  parseFrontmatterBool,
  resolveJARVISManifestBlock,
  resolveJARVISManifestInstall,
  resolveJARVISManifestOs,
  resolveJARVISManifestRequires,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseJARVISManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolveJARVISMetadata(
  frontmatter: ParsedSkillFrontmatter,
): JARVISSkillMetadata | undefined {
  const metadataObj = resolveJARVISManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveJARVISManifestRequires(metadataObj);
  const install = resolveJARVISManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveJARVISManifestOs(metadataObj);
  return {
