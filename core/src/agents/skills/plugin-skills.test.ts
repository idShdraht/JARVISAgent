    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "jarvis.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "jarvis.plugin.json"),
      },
    ],
  };
}
...
describe("resolvePluginSkillDirs", () => {
  it("keeps acpx plugin skills when ACP is enabled", async () => {
    const workspaceDir = await tempDirs.make("jarvis-");
    const acpxRoot = await tempDirs.make("jarvis-acpx-plugin-");
    const helperRoot = await tempDirs.make("jarvis-helper-plugin-");
