...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses JARVIS_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      JARVIS_HOME: "/srv/jarvis-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/jarvis-home"), ".jarvis", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".jarvis", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("jarvis-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-workspace-hardlink-"));
    try {
