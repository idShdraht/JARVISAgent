...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("jarvis-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("jarvis-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("jarvis-sbx-");
