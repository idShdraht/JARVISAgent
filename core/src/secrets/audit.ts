...
function collectAuthStorePaths(config: JARVISConfig, stateDir: string): string[] {
...
export async function runSecretsAudit(
  ...
  const previousAuthStoreReadOnly = process.env.JARVIS_AUTH_STORE_READONLY;
  ...
  try {
    ...
    if (previousAuthStoreReadOnly === undefined) {
      delete process.env.JARVIS_AUTH_STORE_READONLY;
    } else {
      process.env.JARVIS_AUTH_STORE_READONLY = previousAuthStoreReadOnly;
    }
  }
}
