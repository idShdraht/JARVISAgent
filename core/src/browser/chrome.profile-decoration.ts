...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".jarvis-profile-decorated");
}
 ...
 */
export function decorateJARVISProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_JARVIS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_JARVIS_BROWSER_COLOR).toUpperCase();
