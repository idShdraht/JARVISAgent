  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          jarvis: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.JARVIS_GATEWAY_PORT;
    process.env.JARVIS_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.JARVIS_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.JARVIS_GATEWAY_PASSWORD;
