...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredJARVISTmpDir: vi.fn(() => "/tmp/jarvis"),
}));
vi.mock("../infra/tmp-jarvis-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredJARVISTmpDir.mockReturnValue("/tmp/jarvis");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredJARVISTmpDir.mockReturnValue("/tmp/jarvis-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "jarvis-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredJARVISTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "jarvis-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "jarvis-preferred", "downloads")}${path.sep}`),
    );
  });
