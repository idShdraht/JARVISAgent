...
const RELAY_TOKEN_CONTEXT = "jarvis-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const JARVIS_RELAY_BROWSER = "JARVIS/extension-relay";

function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.JARVIS_GATEWAY_TOKEN?.trim() || process.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or JARVIS_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedJARVISRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === JARVIS_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
