...
function listAgents(cfg: JARVISConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: JARVISConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: JARVISConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<JARVISConfig, EvaluatedBindingsCache>();
