...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["JARVIS_BUNDLED_HOOKS_DIR"]);
    process.env.JARVIS_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
