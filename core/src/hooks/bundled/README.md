...
This directory contains hooks that ship with JARVIS. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.jarvis/jarvis.json`:
...
2. Restart gateway: `pkill -9 -f 'jarvis.*gateway' && pnpm jarvis gateway`
3. Enable the hook: `jarvis hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.jarvis.ai/automation/hooks
