...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses JARVIS_HOME for tilde expansion", () => {
    vi.stubEnv("JARVIS_HOME", "/srv/jarvis-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/jarvis-home", "cron", "jobs.json"));
  });
});
