...
describe("canvas host state dir defaults", () => {
  it("uses JARVIS_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("jarvis-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("JARVIS Canvas");
      } finally {
        await handler.close();
      }
    });
