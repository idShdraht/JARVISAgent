...
function resolveVoiceTtsConfig(params: { cfg: JARVISConfig; override?: TtsConfig }): {
  cfg: JARVISConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredJARVISTmpDir(), "discord-voice-"));
