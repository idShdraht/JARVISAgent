...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "jarvis.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "jarvis.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.JARVIS_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.JARVIS_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.JARVIS_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.JARVIS_NODE_OPTIONS_READY = "1";
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[jarvis] ${parsed.error}`);
      process.exit(2);
    }
