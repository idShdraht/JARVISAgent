...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "jarvis_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "jarvis_cmdarg", "arg-menu options");
  });
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(testHarness.actions.has("jarvis_cmdarg")).toBe(true);
    expect(testHarness.options.has("jarvis_cmdarg")).toBe(true);
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("jarvis_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("jarvis_cmdarg");
    expect(blockId).toContain("jarvis_cmdarg_ext:");
    const token = (blockId ?? "").slice("jarvis_cmdarg_ext:".length);
