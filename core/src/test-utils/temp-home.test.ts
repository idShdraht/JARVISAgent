...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const previousStateDir = process.env.JARVIS_STATE_DIR;

    const tempHome = await createTempHomeEnv("jarvis-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.JARVIS_STATE_DIR).toBe(path.join(tempHome.home, ".jarvis"));
    ...
    expect(process.env.JARVIS_STATE_DIR).toBe(previousStateDir);
