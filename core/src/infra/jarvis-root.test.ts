...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__jarvis_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "jarvis-root");
...
describe("resolveJARVISPackageRoot", () => {
  ...
  let resolveJARVISPackageRootSync: typeof import("./jarvis-root.js").resolveJARVISPackageRootSync;

  beforeAll(async () => {
    ({ resolveJARVISPackageRoot, resolveJARVISPackageRootSync } =
      await import("./jarvis-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "jarvis");
    const pkgRoot = path.join(project, "node_modules", "jarvis");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "jarvis" }));
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "jarvis");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "jarvis.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "jarvis" }));
  ...
  it("returns null for non-jarvis package roots", async () => {
    const pkgRoot = fx("not-jarvis");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-jarvis" }));
