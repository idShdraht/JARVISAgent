...
export function triggerJARVISRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.JARVIS_SYSTEMD_UNIT,
        process.env.JARVIS_PROFILE,
      );
  ...
  const label =
    process.env.JARVIS_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.JARVIS_PROFILE);
