...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.JARVIS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*jarvis-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "jarvis";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "jarvis");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "jarvis-gw");
  ...
  records.push(`_jarvis-gw._tcp IN PTR ${instanceLabel}._jarvis-gw._tcp`);
  records.push(`${instanceLabel}._jarvis-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._jarvis-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; jarvis-content-hash: ${contentHash}\n${contentBody}`;
}
