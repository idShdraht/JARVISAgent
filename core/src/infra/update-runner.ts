...
const DEFAULT_PACKAGE_NAME = "jarvis";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "jarvis.mjs");
    ...
    const doctorStep = await runStep(
      step("jarvis doctor", doctorArgv, gitRoot, { JARVIS_UPDATE_IN_PROGRESS: "1" }),
    );
