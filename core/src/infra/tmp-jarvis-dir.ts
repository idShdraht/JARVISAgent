...
export const POSIX_JARVIS_TMP_DIR = "/tmp/jarvis";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredJARVISTmpDirOptions = {
...
export function resolvePreferredJARVISTmpDir(
  options: ResolvePreferredJARVISTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "jarvis" : `jarvis-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[jarvis] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback JARVIS temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback JARVIS temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_JARVIS_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_JARVIS_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_JARVIS_TMP_DIR)) {
      return POSIX_JARVIS_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_JARVIS_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_JARVIS_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_JARVIS_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_JARVIS_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_JARVIS_TMP_DIR;
