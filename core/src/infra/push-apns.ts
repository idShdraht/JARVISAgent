...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.JARVIS_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.JARVIS_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set JARVIS_APNS_TEAM_ID and JARVIS_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.JARVIS_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.JARVIS_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.JARVIS_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set JARVIS_APNS_PRIVATE_KEY_P8 or JARVIS_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading JARVIS_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createJARVISPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    jarvis: createJARVISPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
