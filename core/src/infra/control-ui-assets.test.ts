...
vi.mock("./jarvis-root.js", () => ({
  resolveJARVISPackageRoot: vi.fn(async () => null),
  resolveJARVISPackageRootSync: vi.fn(() => null),
}));
...
let jarvisRoot: typeof import("./jarvis-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    jarvisRoot = await import("./jarvis-root.js");
  });
  ...
  it("uses resolveJARVISPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/jarvis");
    (
      jarvisRoot.resolveJARVISPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/jarvis"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "jarvis" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "jarvis.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-jarvis");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/jarvis-bundle");
    (
      jarvisRoot.resolveJARVISPackageRootSync as unknown as ReturnType<typeof vi.fn>
