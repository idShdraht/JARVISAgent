...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: JARVISConfig | null = null;
  ...
  get(key: string, cfg: JARVISConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: JARVISConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: JARVISConfig): void {
