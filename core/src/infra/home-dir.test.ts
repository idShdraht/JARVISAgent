...
describe("resolveEffectiveHomeDir", () => {
  it("prefers JARVIS_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands JARVIS_HOME when set to ~", () => {
    const env = {
      JARVIS_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for JARVIS_HOME", () => {
    const result = resolveRequiredHomeDir(
      { JARVIS_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when JARVIS_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ JARVIS_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { JARVIS_HOME: "/srv/jarvis-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/jarvis-home")}/x`);
  });
