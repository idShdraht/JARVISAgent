...
describe("resolveGatewayConnection", () => {
  ...
  it("uses JARVIS_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ JARVIS_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers JARVIS_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ JARVIS_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
