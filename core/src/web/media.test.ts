...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredJARVISTmpDir(), "jarvis-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub JARVIS_STATE_DIR.
    // Also keep it outside the JARVIS temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["JARVIS_STATE_DIR"]);
    process.env.JARVIS_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "jarvis-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredJARVISTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default JARVIS state workspace and sandbox roots", async () => {
  ...
  it("rejects default JARVIS state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
