...
import Ajv from "ajv";
import { resolvePreferredJARVISTmpDir } from "jarvis/plugin-sdk";
...
import type { JARVISPluginApi } from "../../../src/plugins/types.js";
...
export function createLlmTaskTool(api: JARVISPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredJARVISTmpDir(), "jarvis-llm-task-"),
        );
