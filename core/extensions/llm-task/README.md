...
`jarvis.invoke --each`) without adding new JARVIS code per workflow.
...
This extension depends on JARVIS internal modules (the embedded agent runner).
It is intended to ship as a **bundled** JARVIS extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.jarvis/extensions` as a standalone plugin directory.
