...
vi.mock("jarvis/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("jarvis/plugin-sdk")>("jarvis/plugin-sdk");
  return {
    ...actual,
...
import type { JARVISPluginServiceContext } from "jarvis/plugin-sdk";
import { emitDiagnosticEvent } from "jarvis/plugin-sdk";
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(
      telemetryState.histograms.get("jarvis.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    ...
    expect(
      telemetryState.histograms.get("jarvis.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    ...
    expect(
      telemetryState.histograms.get("jarvis.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
