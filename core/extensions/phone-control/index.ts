...
import type { JARVISPluginApi, JARVISPluginService } from "jarvis/plugin-sdk";
...
function normalizeDenyList(cfg: JARVISPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: JARVISPluginApi["config"]): string[] {
...
export default function register(api: JARVISPluginApi) {
  ...
  const timerService: JARVISPluginService = {
