...
JARVIS extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
jarvis channels login --channel zalouser
...
```bash
jarvis message send --channel zalouser --target <threadId> --message "Hello from JARVIS!"
```
...
```bash
jarvis channels login --channel zalouser              # Login via QR
jarvis channels login --channel zalouser --account work
jarvis channels status --probe
jarvis channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
jarvis directory self --channel zalouser
jarvis directory peers list --channel zalouser --query "name"
jarvis directory groups list --channel zalouser --query "work"
jarvis directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
jarvis message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
jarvis message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `jarvis channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
jarvis channels login --channel zalouser --account work
jarvis message send --channel zalouser --account work --target <id> --message "Hello"
