...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.JARVIS_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "jarvis-nostr-"));
  process.env.JARVIS_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const stateEnv = env ?? process.env;
        const override = stateEnv.JARVIS_STATE_DIR?.trim() || stateEnv.CLAWDBOT_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        const resolveHome = homedir ?? os.homedir;
        return path.join(resolveHome(), ".jarvis");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.JARVIS_STATE_DIR = previous;
    }
