# Google Gemini CLI Auth (JARVIS plugin)
...
```bash
jarvis plugins enable google-gemini-cli-auth
```
...
```bash
jarvis models auth login --provider google-gemini-cli --set-default
```
...
- `JARVIS_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `JARVIS_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
