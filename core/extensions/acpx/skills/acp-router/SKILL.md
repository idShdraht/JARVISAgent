---
name: acp-router
description: Route plain-language requests for Pi, Claude Code, Codex, OpenCode, Gemini CLI, or ACP harness work into either JARVIS ACP runtime sessions or direct acpx-driven sessions ("telephone game" flow).
...
Trigger this skill when the user asks JARVIS to:
...
1. JARVIS ACP runtime path (default): use `sessions_spawn` / ACP runtime tools.
...
- the task is "just relay prompts to harness" and no JARVIS ACP lifecycle features are needed
...
For this repo, direct `acpx` calls must follow the same pinned policy as the `@jarvis/acpx` extension.
