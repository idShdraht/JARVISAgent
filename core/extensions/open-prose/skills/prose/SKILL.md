...
metadata: { "jarvis": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == JARVIS `sessions_spawn`
- **File I/O** == JARVIS `read`/`write`
- **Remote fetch** == JARVIS `web_fetch` (or `exec` with curl when POST is required)
