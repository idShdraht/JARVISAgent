import type { AnyAgentTool, JARVISPluginApi } from "jarvis/plugin-sdk";
...
export function createToolFactoryHarness(cfg: JARVISPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<JARVISPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
