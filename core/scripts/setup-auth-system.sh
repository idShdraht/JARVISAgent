#!/bin/bash
# Setup JARVIS Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'jarvis-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  JARVIS message: Send warning via JARVIS itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/jarvis-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/jarvis-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/jarvis-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now jarvis-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status jarvis-auth-monitor.timer"
echo ""
echo "Quick commands:"
