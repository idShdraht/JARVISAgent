 ...
 * `jarvis update`. Use it to verify the completion installation flow
...
\`jarvis update\`, \`jarvis doctor\`, and \`jarvis onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# JARVIS Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`jarvis completion --install\` later to enable.`));
    return;
  }
