#!/usr/bin/env bash
# Rootless JARVIS in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./setup-podman.sh
# Then:
#   ./scripts/run-jarvis-podman.sh launch           # Start gateway
...
LAUNCH_SCRIPT="$JARVIS_HOME/run-jarvis-podman.sh"
...
if [[ "${1:-}" == "launch" ]]; then
  shift
  if [[ -n "${JARVIS_UID:-}" && "$(id -u)" -ne "$JARVIS_UID" ]]; then
    # Exec as jarvis with cwd=/tmp so a nologin user never inherits an invalid cwd.
    exec sudo -u "$JARVIS_USER" env HOME="$JARVIS_HOME" PATH="$PATH" TERM="${TERM:-}" \
      bash -c 'cd /tmp && exec '"$LAUNCH_SCRIPT"' "$@"' _ "$@"
  fi
  # Already jarvis; fall through to container run (with remaining args, e.g. "setup")
fi

# --- Container run (script in jarvis home, run as jarvis) ---
EFFECTIVE_HOME="${HOME:-}"
if [[ -n "${JARVIS_UID:-}" && "$(id -u)" -eq "$JARVIS_UID" ]]; then
  EFFECTIVE_HOME="$JARVIS_HOME"
  export HOME="$JARVIS_HOME"
fi
if [[ -z "${EFFECTIVE_HOME:-}" ]]; then
  EFFECTIVE_HOME="${JARVIS_HOME:-/tmp}"
fi
CONFIG_DIR="${JARVIS_CONFIG_DIR:-$EFFECTIVE_HOME/.jarvis}"
ENV_FILE="${JARVIS_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${JARVIS_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${JARVIS_PODMAN_CONTAINER:-jarvis}"
JARVIS_IMAGE="${JARVIS_PODMAN_IMAGE:-jarvis:local}"
PODMAN_PULL="${JARVIS_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${JARVIS_PODMAN_GATEWAY_HOST_PORT:-${JARVIS_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${JARVIS_PODMAN_BRIDGE_HOST_PORT:-${JARVIS_BRIDGE_PORT:-18790}}"
...
GATEWAY_BIND="${JARVIS_GATEWAY_BIND:-loopback}"
...
if [[ -z "${JARVIS_GATEWAY_TOKEN:-}" ]]; then
  export JARVIS_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "JARVIS_GATEWAY_TOKEN" "$JARVIS_GATEWAY_TOKEN"
  echo "Generated JARVIS_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

# The gateway refuses to start unless gateway.mode=local is set in config.
# Keep this minimal; users can run the wizard later to configure channels/providers.
CONFIG_JSON="$CONFIG_DIR/jarvis.json"
...
PODMAN_USERNS="${JARVIS_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported JARVIS_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (JARVIS_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi
...
if [[ "$RUN_SETUP" == true ]]; then
  exec podman run --pull="$PODMAN_PULL" --rm -it \
    ...
    -e JARVIS_GATEWAY_TOKEN="$JARVIS_GATEWAY_TOKEN" \
    -v "$CONFIG_DIR:/home/node/.jarvis:rw" \
    -v "$WORKSPACE_DIR:/home/node/.jarvis/workspace:rw" \
    "${ENV_FILE_ARGS[@]}" \
    "$JARVIS_IMAGE" \
