...
show_usage() {
    cat << EOF
clawlog - JARVIS Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View JARVIS logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    JARVIS logs flow through the macOS unified log (subsystem: ai.jarvis).
