...
Inspired by Simon Willison's [Running JARVIS in Docker](https://til.simonwillison.net/llms/jarvis-docker).
...
On first command, ClawDock auto-detects your JARVIS directory:

- Checks common paths (`~/jarvis`, `~/workspace/jarvis`, etc.)
...
| `clawdock-cli <command>`  | Run JARVIS CLI commands                      |
...
| `clawdock-cd`        | Jump to the JARVIS project directory    |
| `clawdock-config`    | Open the JARVIS config directory        |
...
```bash
jarvis channels login --channel whatsapp --verbose
```
...
2. Configure it in the JARVIS config
...
- Bash or Zsh shell
- JARVIS project (from `docker-setup.sh`)
