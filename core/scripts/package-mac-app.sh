...
# Build and bundle JARVIS into a minimal .app we can open.
# Outputs to dist/JARVIS.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/JARVIS.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="JARVIS"
BUNDLE_ID="${BUNDLE_ID:-ai.jarvis.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/jarvis/jarvis/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/JARVIS/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :JARVISBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :JARVISGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/JARVIS"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/JARVIS"
fi
chmod +x "$APP_ROOT/Contents/MacOS/JARVIS"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/JARVIS" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/JARVIS/Resources/JARVIS.icns" "$APP_ROOT/Contents/Resources/JARVIS.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/JARVIS/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying JARVISKit resources"
JARVISKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/JARVISKit_JARVISKit.bundle"
if [ -d "$JARVISKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/JARVISKit_JARVISKit.bundle"
  cp -R "$JARVISKIT_BUNDLE" "$APP_ROOT/Contents/Resources/JARVISKit_JARVISKit.bundle"
else
  echo "WARN: JARVISKit resource bundle not found at $JARVISKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running JARVIS"
killall -q JARVIS 2>/dev/null || true
