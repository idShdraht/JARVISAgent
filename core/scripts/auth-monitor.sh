...
#
# Suggested cron: */30 * * * * /home/admin/jarvis/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send JARVIS notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., jarvis-alerts)
...
STATE_FILE="$HOME/.jarvis/auth-monitor-state"
...
send_notification() {
    ...
    # Send via JARVIS if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use jarvis
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via JARVIS to $NOTIFY_PHONE..."
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! JARVIS is down. Run: ssh l36 '~/jarvis/scripts/mobile-reauth.sh'" "urgent"
    exit 1
