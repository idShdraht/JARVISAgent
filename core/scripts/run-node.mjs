...
const shouldBuild = (deps) => {
  if (deps.env.JARVIS_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.JARVIS_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[jarvis] ${message}\n`);
};

const runJARVIS = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["jarvis.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
