...
IMAGE_NAME="jarvis-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="jarvis-net-e2e-$$"
GW_NAME="jarvis-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "JARVIS_GATEWAY_TOKEN=$TOKEN" \
  -e "JARVIS_SKIP_CHANNELS=1" \
  -e "JARVIS_SKIP_GMAIL_WATCHER=1" \
  -e "JARVIS_SKIP_CRON=1" \
  -e "JARVIS_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
