#!/usr/bin/env bash
# Reset JARVIS like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${JARVIS_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="JARVIS.app/Contents/MacOS/JARVIS"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/JARVIS"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/JARVIS"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/JARVIS"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.jarvis.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/jarvis-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${JARVIS_RESTART_LOG:-/tmp/jarvis-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${JARVIS_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.jarvis/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  JARVIS_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node jarvis.mjs daemon install --force --runtime node"
      log "  node jarvis.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.jarvis/disable-launchagent"
      log ""
...
kill_all_jarvis() {
  for _ in {1..10}; do
    ...
    if ! pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1 \
       ...
       && ! pgrep -x "JARVIS" >/dev/null 2>&1; then
      return 0
    fi
    sleep 0.3
  done
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.jarvis.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing JARVIS instances"
kill_all_jarvis
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product JARVIS"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/JARVIS.app" ]]; then
    APP_BUNDLE="/Applications/JARVIS.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/JARVIS.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/JARVIS.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/JARVIS.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set JARVIS_APP_BUNDLE to your installed JARVIS.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node jarvis.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node jarvis.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".jarvis", "jarvis.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: JARVIS is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.jarvis.gateway.plist' | head -n 40 || true"
fi
