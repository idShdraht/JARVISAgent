...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/jarvis/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "JARVIS Re-Auth" -c "After getting key, run: ssh $SERVER '~/jarvis/scripts/mobile-reauth.sh'" --id jarvis-auth
        ;;
    *)
