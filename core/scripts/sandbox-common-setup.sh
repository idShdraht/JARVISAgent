...
BASE_IMAGE="${BASE_IMAGE:-jarvis-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-jarvis-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=jarvis.sandbox=1)
NOTE
