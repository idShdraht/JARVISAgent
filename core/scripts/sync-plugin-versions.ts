...
function ensureChangelogEntry(changelogPath: string, version: string): boolean {
  ...
  const entry = `## ${version}\n\n### Changes\n- Version alignment with core JARVIS release numbers.\n\n`;
...
function stripWorkspaceJarvisDevDependency(pkg: PackageJson): boolean {
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceJarvisDevDependency(pkg);
