...
const urlRaw = getArg("--url") ?? process.env.JARVIS_GATEWAY_URL;
const token = getArg("--token") ?? process.env.JARVIS_GATEWAY_TOKEN;
const nodeHint = getArg("--node");
const dangerous = hasFlag("--dangerous") || process.env.JARVIS_RUN_DANGEROUS === "1";
...
if (!urlRaw || !token) {
  ...
  console.error(
    "Usage: bun scripts/dev/ios-node-e2e.ts --url <wss://host[:port]> --token <gateway.auth.token> [--node <id|name-substring>] [--dangerous] [--json]\n" +
      "Or set env: JARVIS_GATEWAY_URL / JARVIS_GATEWAY_TOKEN",
  );
  process.exit(1);
}
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      ...
      platform: "dev",
      mode: "cli",
      instanceId: "jarvis-dev-ios-node-e2e",
    },
    locale: "en-US",
