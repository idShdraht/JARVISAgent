...
INSTALL_URL="${JARVIS_INSTALL_URL:-https://jarvis.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${JARVIS_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${JARVIS_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="jarvis"
PACKAGE_NAME="${JARVIS_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (JARVIS_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${JARVIS_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${JARVIS_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
