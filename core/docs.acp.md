...
This document describes how the JARVIS ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`jarvis acp` exposes an ACP agent over stdio and forwards prompts to a running
JARVIS Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a JARVIS Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `jarvis acp` over stdio.

Example config:

```bash
jarvis config set gateway.remote.url wss://gateway-host:18789
jarvis config set gateway.remote.token <token>
```
...
jarvis acp --session agent:design:main
jarvis acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “JARVIS ACP” to start a thread.

## Execution Model

- ACP client spawns `jarvis acp` and speaks ACP messages over stdio.
...
```bash
jarvis acp --session agent:main:main
jarvis acp --session-label "support inbox"
jarvis acp --reset-session
```
...
`jarvis acp` resolves the Gateway URL and auth from CLI flags or config:
