---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for JARVIS deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, JARVIS cron scheduling for periodic checks, or version status checks on a machine running JARVIS (laptop, workstation, Pi, VPS).
---

# JARVIS Host Hardening

## Overview

Assess and harden the host running JARVIS, then align it to a user-defined risk tolerance without breaking access. Use JARVIS security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim JARVIS changes the host firewall, SSH, or OS updates; it does not.
...
5. JARVIS gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if JARVIS can access sensitive data.
...
### 2) Run JARVIS security audits (read-only)

As part of the default read-only checks, run `jarvis security audit --deep`. Only offer alternatives if the user requests them:

1. `jarvis security audit` (faster, non-probing)
2. `jarvis security audit --json` (structured output)

Offer to apply JARVIS safe defaults (numbered):

1. `jarvis security audit --fix`

Be explicit that `--fix` only tightens JARVIS defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check JARVIS version/update status (read-only)

As part of the default read-only checks, run `jarvis update status`.
...
- Credential hygiene notes (location of JARVIS creds, prefer disk encryption)
...
After JARVIS install or first hardening pass, run at least one baseline audit and version check:

- `jarvis security audit`
- `jarvis security audit --deep`
- `jarvis update status`

Ongoing monitoring is recommended. Use the JARVIS cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `jarvis cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `jarvis update status`
...
Before creating, `jarvis cron list` and match on exact `name`. If found, `jarvis cron edit <id> ...`.
If not found, `jarvis cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `jarvis update status` (preferred for source checkouts and channels)
2. `npm view jarvis version` (published npm version)

## JARVIS command accuracy

Use only supported commands and flags:

- `jarvis security audit [--deep] [--fix] [--json]`
- `jarvis status` / `jarvis status --deep`
- `jarvis health --json`
- `jarvis update status`
- `jarvis cron add|list|runs|run`

Do not invent CLI flags or imply JARVIS enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by JARVIS compaction:
