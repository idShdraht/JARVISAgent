...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `jarvis-e2e-${name}-`));
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          JARVIS_SKIP_PROVIDERS: "1",
          JARVIS_SKIP_GMAIL_WATCHER: "1",
          JARVIS_SKIP_CRON: "1",
          JARVIS_SKIP_BROWSER_CONTROL_SERVER: "1",
          JARVIS_SKIP_CANVAS_HOST: "1",
          JARVIS_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
