...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/jarvis/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/jarvis");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/jarvis/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__JARVIS_CONTROL_UI_BASE_PATH__ = "/jarvis";
    const app = mountApp("/jarvis/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/jarvis");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/jarvis/sessions");
  });
