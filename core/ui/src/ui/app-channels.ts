...
export async function handleWhatsAppStart(host: JARVISApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: JARVISApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: JARVISApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: JARVISApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: JARVISApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: JARVISApp): string {
...
function resolveGatewayHttpAuthHeader(host: JARVISApp): string | null {
...
function buildGatewayHttpHeaders(host: JARVISApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: JARVISApp) {
...
export function handleNostrProfileToggleAdvanced(host: JARVISApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: JARVISApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: JARVISApp) {
  const state = host.nostrProfileFormState;
