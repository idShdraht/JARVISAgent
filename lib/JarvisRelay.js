...
export class JarvisRelay {
    constructor(config) {
        ...
        this.openclawPath = './openclaw'; // Path to the underlying engine
    }
    ...
    async onMessageReceived(platform, senderId, text, rawData = {}) {
        ...
        try {
            ...
            // 2. Get AI Response from OpenClaw
    ...
    /**
     * Interfaces with the underlying OpenClaw engine
     */
    async getEngineResponse(input) {
        return new Promise((resolve, reject) => {
            // Mocking openclaw call — in reality, this would be an exec or an internal lib call
            // We white-label it as JARVIS
            const cmd = `echo "${input}" | ${this.openclawPath} --agent=jarvis`;
